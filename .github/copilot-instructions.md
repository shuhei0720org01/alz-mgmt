...
- ✅ Chapter 06: 設定テンプレート（完了・本番）
- ✅ Chapter 07: リソースグループ（完了・本番）
- ✅ Chapter 08: 管理グループとポリシー（v3完了・本番待ち）
- ⏳ Chapter 09: 管理リソース（v3作成予定）
- ⏳ Chapter 10: Hub-and-Spoke（v3作成予定）
...
### Chapter 07校正メモ（2026年1月22日追加）

以下の校正ルールを他のChapterでも適用すること：

**実践パターンの構成：**

- 複数の実践パターンは1つのPartにまとめる
- 各パターンを「パターン1」「パターン2」として分ける
- 基本 → 応用の順に並べる

**現状の制限を明示：**

- main.tfで渡していないパラメータは「修正が必要」と明記
- 修正前と修正後のコードを両方示す
- 設定ファイルでの指定方法も具体的に
...
- 公式モジュール解説の前にスクリーンショットを追加
- 「ここからは公式のモジュールを開きながら見ていきましょう」のような誘導文
- モジュールとドキュメントを並べて表示する画像（例：`![モジュールとドキュメントを並べて見る](img/スクリーンショット2.png)`）

**URLの扱い：**

- URLは直接記述（コードブロックで囲まない）
- URLの前後に適切な空行を入れる
- 例：
  ```
  **GitHubリポジトリ：**
...
### Chapter 08校正メモ（2026年1月22日追加）

以下の校正ルールを他のChapterでも適用すること：

**公式モジュールの解説：**

- 概念的な説明ではなく、実際のコードを詳細に解説すること

**解説の粒度：**

- Chapter 07と同レベルの詳細度を保つ
- 「概念」ではなく「実際のコード」を示す
- ユーザーが「全然理解できない」とならないレベル



**重要**: 公式モジュールを参照している場合、そのモジュールの内部コードまで完全に解説すること。

**対象:**

- `source = "Azure/xxx/azurerm"` 形式の公式モジュール
- `./modules/xxx` 形式のローカルモジュール

**解説手順:**

1. **モジュールのソースコードを取得**:
   - 公式モジュール: GitHubリポジトリから該当バージョンのコードを読む
   - ローカルモジュール: 実際のファイルを読む

2. **モジュールの構成を解説**:
   - main.tf、variables.tf、outputs.tf等の主要ファイル
   - 作成されるリソースの一覧
   - 主要な変数とその役割
...
**Chapter 07-11のv3作成が必要:**

- Chapter 07: azurerm_resource_groupリソース（公式モジュールは使っていないが念のため確認）
- Chapter 08: modules/management_groups/の内部解説が必要
- Chapter 09: modules/management_resources/の内部解説が必要  
- Chapter 10: Azure/avm-ptn-alz-connectivity-hub-and-spoke-vnet/azurermの内部解説が必要
- Chapter 11: 同様に公式モジュールの内部解説が必要
