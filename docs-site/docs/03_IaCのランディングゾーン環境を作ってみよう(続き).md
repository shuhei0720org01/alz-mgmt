...
※もしエラーが出た場合は、何度か再実行したら大体成功します。それでもダメな場合はエラーを読んで、わからない場合はAIに聞いてみましょう。

※一度AzureFireallポリシーがずっとupdatingになって進まないことがありました。この場合AzureFireallポリシーが消せずに残り続けて最悪です。
...
次に、「lib/archetype_definitions/landing_zones_custom.alz_archetype_override.yaml」も同様にDDOSのポリシーを無効にしてください。
...
```
# terraformのフォーマット確認
sudo rm -f /etc/apt/sources.list.d/yarn.list && curl -fsSL https://apt.releases.hashicorp.com/gpg | sudo gpg --yes --dearmor -o /usr/share/keyrings/hashicorp-archive-keyring.gpg && echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/hashicorp.list > /dev/null && sudo apt update -qq 2>/dev/null || true && sudo apt install -y terraform && terraform fmt -recursive . && echo "✅ Complete!"
...
※Connectivity管理グループとLandingzones管理グループにもともと割り当てられていましたがなくなっています。
