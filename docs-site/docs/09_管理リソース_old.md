...
```hcl title="Log Analyticsワークスペース名"
log_analytics_workspace_name = "law-management-japaneast"
```
...
#### user_assigned_managed_identities
```hcl title="ユーザー割り当てManaged Identity"
user_assigned_managed_identities = {
  ama = {
    name = "id-ama-japaneast"
  }
...
##### 2. defender_sql
```text title="SQL Serverのセキュリティ監視"
...
#### providers
```hcl title="Managementサブスクリプション指定"
providers = {
  azurerm = azurerm.management
}
```
...
```hcl title="Automation Accountの作成制御"
linked_automation_account_creation_enabled = false
```
...
**注意**：
クォータに達するとログが取り込まれないから、重要なログが欠損する可能性あり。
...
```text title="Defender for SQL DCRの収集対象"
収集するデータ：
...
```text title="VM Insights DCRの収集対象"
収集するデータ：
...
**Managed Identityの方法**：
```text title="Managed Identityでの認証フロー"
...
このプロジェクトではUser-assignedを使ってる。

#### 使われ方

```text title="Managed Identityの使用フロー"
...
```hcl title="複数のManaged Identityの定義"
user_assigned_managed_identities = {
  ama = {
    name = "id-ama-japaneast"
  }
...
```bash title="Log Analytics Workspaceの確認コマンド"
# ワークスペース一覧
...
```bash title="Data Collection Ruleの確認コマンド"
# DCR一覧
...
```bash title="Managed Identityの確認コマンド"
# Managed Identity一覧
...
```kql title="Log Analyticsでのログ確認クエリ"
// Heartbeat（VM生存確認）
Heartbeat
| where TimeGenerated > ago(1h)
...
### 問題1
Log Analyticsワークスペースのデータ保持期間を90日に設定するには、  
どのプロパティをどう設定しますか？

### 問題2
Data Collection Rule（DCR）の役割は何ですか？

### 問題3
User Assigned Managed Identityを使う理由は何ですか？
...
### 答え1
**`retention_in_days = 90`**を設定します。

```hcl title="Log Analyticsの保持期間設定"
resource "azurerm_log_analytics_workspace" "management" {
  name                = "log-management-jp"
  location            = "japaneast"
  resource_group_name = azurerm_resource_group.management.name
  sku                 = "PerGB2018"
  retention_in_days   = 90  # ← これ
}
```

### 答え2
**VMからLog Analyticsへ送信するログの種類とフィルタリングルールを定義**します。

```
VM → DCR（フィルタリング）→ Log Analytics
```

DCRで以下を設定：
- どのログを収集するか（Performance、Syslog等）
- どのLog Analyticsに送るか
- データの変換ルール

### 答え3
**複数のリソースから同じIDを共有して使えるから**です。

```hcl
# System Assigned（リソースごとに1つ）
resource "azurerm_virtual_machine" "vm1" {
  identity { type = "SystemAssigned" }  # VM1専用のID
}
...
resource "azurerm_virtual_machine" "vm1" {
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.shared.id]  # 共有
  }
}
...
resource "azurerm_virtual_machine" "vm2" {
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.shared.id]  # 同じID
  }
}
```
