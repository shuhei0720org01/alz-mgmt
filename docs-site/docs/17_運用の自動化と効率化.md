# 17. 運用の自動化と効率化

!!! info "この章で学ぶこと"
    運用を自動化して効率化する方法を学びます：

    1. 運用自動化の設計
    2. モニタリングとアラート
    3. コスト最適化の実践
    4. 運用ベストプラクティス

    この章で、効率的な運用体制を構築できます。

---

## 運用自動化の全体像

Landing Zonesの運用自動化の流れを視覚化してみましょう。

```mermaid
graph TB
    A[運用タスク] --> B[定期実行<br/>Scheduled Workflow]
    A --> C[イベント駆動<br/>Event-driven]
    A --> D[手動実行<br/>Manual Trigger]
    
    B --> B1[毎日: コンプライアンスチェック]
    B --> B2[毎週: コスト分析レポート]
    B --> B3[毎月: セキュリティスキャン]
    
    C --> C1[PR作成: Plan自動実行]
    C --> C2[マージ: Apply自動実行]
    C --> C3[リソース作成: アラート送信]
    
    D --> D1[緊急変更: 手動Apply]
    D --> D2[ロールバック: 手動実行]
    
    B1 --> E[Azure Monitor]
    B2 --> E
    B3 --> E
    C3 --> E
    
    E --> F[アラート評価]
    F --> G{閾値超過?}
    
    G -->|Yes| H[Action Group起動]
    G -->|No| I[正常: 記録のみ]
    
    H --> H1[Email通知]
    H --> H2[Slack/Teams通知]
    H --> H3[Logic Apps実行]
    H --> H4[Webhook呼び出し]
    
    H1 --> J[管理者: 対応]
    H2 --> J
    H3 --> J
    H4 --> J
    
    J --> K[問題解決]
    K --> L[ログ記録・ドキュメント更新]
    
    style A fill:#e1f5ff
    style E fill:#fff4e1
    style G fill:#ffe1e1
    style J fill:#ffebe1
    style L fill:#e1ffe1
```

**自動化の3つの柱**：

1. **定期実行**: Scheduled Workflowで定期的にチェック
2. **イベント駆動**: Git操作やリソース変更で自動実行
3. **監視・アラート**: Azure Monitorで異常を検知

**通知フロー**：

1. Azure Monitorが異常を検知
2. Action Groupが複数の通知先に送信
3. 管理者が対応
4. 解決後、ドキュメントを更新

---

## Part 1: 運用自動化の設計

### 自動化すべきタスクの選定

どのタスクを自動化すべきか判断します。

=== "自動化の優先度"

    **高優先度（すぐに自動化）**:
    
    - ✅ 定期的に実行するタスク
    - ✅ 手順が明確なタスク
    - ✅ ミスが許されないタスク
    - ✅ 時間がかかるタスク
    
    **例**:
    - Policy準拠状況レポート（毎週）
    - バックアップ確認（毎日）
    - コストレポート（毎月）
    - セキュリティスキャン（毎日）

=== "自動化しないタスク"

    **自動化に不向き**:
    
    - ❌ 頻度が低いタスク（年1回など）
    - ❌ 判断が必要なタスク
    - ❌ 手順が不明確なタスク
    - ❌ 自動化コストが高いタスク
    
    **例**:
    - 重大インシデント対応
    - アーキテクチャ設計
    - ビジネス要件のヒアリング

=== "ROI評価"

    | タスク | 頻度 | 手動時間 | 自動化コスト | ROI |
    |--------|------|----------|------------|-----|
    | Policy準拠レポート | 週1 | 30分 | 4時間 | ⭐⭐⭐⭐⭐ |
    | コストレポート | 月1 | 2時間 | 8時間 | ⭐⭐⭐ |
    | バックアップ確認 | 日1 | 10分 | 4時間 | ⭐⭐⭐⭐⭐ |
    | セキュリティスキャン | 日1 | 15分 | 6時間 | ⭐⭐⭐⭐ |

### GitHub Actionsでの自動化

GitHub Actionsで運用タスクを自動化します。

=== "Policy準拠レポート"

    ```yaml title=".github/workflows/policy-compliance-report.yaml"
    name: Policy Compliance Report
    
    on:
      schedule:
        - cron: '0 9 * * 1'  # 毎週月曜 9:00 JST
      workflow_dispatch:
    
    permissions:
      id-token: write
      contents: read
      issues: write
    
    jobs:
      compliance-check:
        runs-on: ubuntu-latest
        steps:
          - name: Azure Login (OIDC)
            uses: azure/login@v1
            with:
              client-id: ${{ secrets.AZURE_CLIENT_ID }}
              tenant-id: ${{ secrets.AZURE_TENANT_ID }}
              subscription-id: ${{ secrets.AZURE_SUBSCRIPTION_ID }}
          
          - name: Get Compliance Status
            id: compliance
            run: |
              az policy state summarize \
                --management-group alz \
                --output json > compliance.json
              
              # 非準拠数を取得
              NON_COMPLIANT=$(jq '.results.nonCompliantResources' compliance.json)
              echo "non_compliant=$NON_COMPLIANT" >> $GITHUB_OUTPUT
          
          - name: Generate Report
            run: |
              cat << EOF > report.md
              # Policy準拠レポート
              
              **日時**: $(date '+%Y-%m-%d %H:%M JST')
              
              ## サマリー
              
              - 非準拠リソース数: ${{ steps.compliance.outputs.non_compliant }}
              
              ## 詳細
              
              \`\`\`json
              $(cat compliance.json)
              \`\`\`
              
              ## アクション
              
              非準拠リソースを確認し、対応してください。
              EOF
          
          - name: Create Issue
            uses: actions/github-script@v6
            with:
              script: |
                const fs = require('fs');
                const report = fs.readFileSync('report.md', 'utf8');
                
                await github.rest.issues.create({
                  owner: context.repo.owner,
                  repo: context.repo.repo,
                  title: `Policy準拠レポート - ${new Date().toLocaleDateString('ja-JP')}`,
                  body: report,
                  labels: ['report', 'policy-compliance']
                });
    ```

=== "バックアップ確認"

    ```yaml title=".github/workflows/backup-check.yaml"
    name: Backup Status Check
    
    on:
      schedule:
        - cron: '0 8 * * *'  # 毎日 8:00 JST
      workflow_dispatch:
    
    permissions:
      id-token: write
      contents: read
    
    jobs:
      backup-check:
        runs-on: ubuntu-latest
        steps:
          - name: Azure Login (OIDC)
            uses: azure/login@v1
            with:
              client-id: ${{ secrets.AZURE_CLIENT_ID }}
              tenant-id: ${{ secrets.AZURE_TENANT_ID }}
              subscription-id: ${{ secrets.AZURE_SUBSCRIPTION_ID }}
          
          - name: Check VM Backups
            run: |
              # Recovery Vaultの確認
              az backup vault list --output table
              
              # バックアップジョブの確認（過去24時間）
              az backup job list \
                --vault-name alz-backup-vault \
                --resource-group alz-management-rg \
                --status Failed \
                --output table > failed-jobs.txt
              
              if [ -s failed-jobs.txt ]; then
                echo "::warning::バックアップジョブが失敗しています"
                cat failed-jobs.txt
                exit 1
              fi
          
          - name: Notify on Failure
            if: failure()
            run: |
              # Slackに通知（後述）
              echo "バックアップ失敗を検知しました"
    ```

=== "未使用リソース検出"

    ```yaml title=".github/workflows/unused-resources.yaml"
    name: Unused Resources Detection
    
    on:
      schedule:
        - cron: '0 10 * * 1'  # 毎週月曜 10:00 JST
      workflow_dispatch:
    
    permissions:
      id-token: write
      contents: read
      issues: write
    
    jobs:
      detect-unused:
        runs-on: ubuntu-latest
        steps:
          - name: Azure Login (OIDC)
            uses: azure/login@v1
            with:
              client-id: ${{ secrets.AZURE_CLIENT_ID }}
              tenant-id: ${{ secrets.AZURE_TENANT_ID }}
              subscription-id: ${{ secrets.AZURE_SUBSCRIPTION_ID }}
          
          - name: Find Unused Public IPs
            id: unused-ips
            run: |
              az network public-ip list \
                --query "[?ipConfiguration==null].{Name:name, RG:resourceGroup, Location:location}" \
                --output json > unused-ips.json
              
              COUNT=$(jq 'length' unused-ips.json)
              echo "count=$COUNT" >> $GITHUB_OUTPUT
          
          - name: Find Unused Disks
            id: unused-disks
            run: |
              az disk list \
                --query "[?diskState=='Unattached'].{Name:name, RG:resourceGroup, Size:diskSizeGb}" \
                --output json > unused-disks.json
              
              COUNT=$(jq 'length' unused-disks.json)
              echo "count=$COUNT" >> $GITHUB_OUTPUT
          
          - name: Create Report Issue
            if: ${{ steps.unused-ips.outputs.count > 0 || steps.unused-disks.outputs.count > 0 }}
            uses: actions/github-script@v6
            with:
              script: |
                const fs = require('fs');
                const unusedIPs = JSON.parse(fs.readFileSync('unused-ips.json', 'utf8'));
                const unusedDisks = JSON.parse(fs.readFileSync('unused-disks.json', 'utf8'));
                
                let body = '# 未使用リソースレポート\n\n';
                body += `**日時**: ${new Date().toLocaleDateString('ja-JP')}\n\n`;
                
                if (unusedIPs.length > 0) {
                  body += '## 未使用Public IP\n\n';
                  body += '| Name | Resource Group | Location |\n';
                  body += '|------|----------------|----------|\n';
                  unusedIPs.forEach(ip => {
                    body += `| ${ip.Name} | ${ip.RG} | ${ip.Location} |\n`;
                  });
                  body += '\n';
                }
                
                if (unusedDisks.length > 0) {
                  body += '## 未使用Disk\n\n';
                  body += '| Name | Resource Group | Size (GB) |\n';
                  body += '|------|----------------|----------|\n';
                  unusedDisks.forEach(disk => {
                    body += `| ${disk.Name} | ${disk.RG} | ${disk.Size} |\n`;
                  });
                }
                
                await github.rest.issues.create({
                  owner: context.repo.owner,
                  repo: context.repo.repo,
                  title: `未使用リソース検出 - ${new Date().toLocaleDateString('ja-JP')}`,
                  body: body,
                  labels: ['report', 'cost-optimization']
                });
    ```

### Scheduled Workflowの活用

定期実行のワークフローを設定します。

=== "cronスケジュール例"

    | 頻度 | cron | 説明 |
    |------|------|------|
    | 毎日8:00 | `0 8 * * *` | バックアップ確認 |
    | 毎週月曜9:00 | `0 9 * * 1` | Policy準拠レポート |
    | 毎月1日10:00 | `0 10 1 * *` | コストレポート |
    | 毎時 | `0 * * * *` | ヘルスチェック |
    
    !!! tip "cron設定"
        cronはUTC時刻です。JST = UTC+9なので、9時間引いた時刻を指定します。
        - JST 9:00 → UTC 0:00 → cron: `0 0 * * *`
        - JST 18:00 → UTC 9:00 → cron: `0 9 * * *`

=== "workflow_dispatch併用"

    ```yaml
    on:
      schedule:
        - cron: '0 9 * * 1'  # 定期実行
      workflow_dispatch:        # 手動実行も可能
        inputs:
          scope:
            description: 'Scope (management-group or subscription)'
            required: false
            default: 'alz'
    ```
    
    定期実行と手動実行の両方に対応します。

### Slack通知の統合

運用タスクの結果をSlackに通知します。

=== "Slack Webhook設定"

    **1. Slack AppでIncoming Webhookを作成**:
    
    - https://api.slack.com/apps → Create New App
    - Incoming Webhooksを有効化
    - Webhook URLを取得
    
    **2. GitHub Secretsに登録**:
    
    ```bash
    gh secret set SLACK_WEBHOOK_URL --body "https://hooks.slack.com/services/..."
    ```

=== "通知ワークフロー"

    ```yaml title=".github/workflows/notify-slack.yaml"
    name: Slack Notification Example
    
    on:
      workflow_dispatch:
    
    jobs:
      notify:
        runs-on: ubuntu-latest
        steps:
          - name: Send to Slack
            uses: slackapi/slack-github-action@v1
            with:
              payload: |
                {
                  "text": "Policy準拠レポート",
                  "blocks": [
                    {
                      "type": "header",
                      "text": {
                        "type": "plain_text",
                        "text": "📊 Policy準拠レポート"
                      }
                    },
                    {
                      "type": "section",
                      "text": {
                        "type": "mrkdwn",
                        "text": "*日時*: $(date '+%Y-%m-%d %H:%M JST')\n*非準拠リソース*: 5件"
                      }
                    },
                    {
                      "type": "section",
                      "text": {
                        "type": "mrkdwn",
                        "text": "<https://github.com/${{ github.repository }}/issues/123|詳細を確認>"
                      }
                    }
                  ]
                }
            env:
              SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
              SLACK_WEBHOOK_TYPE: INCOMING_WEBHOOK
    ```

=== "エラー通知"

    ```yaml
    - name: Notify on Failure
      if: failure()
      uses: slackapi/slack-github-action@v1
      with:
        payload: |
          {
            "text": "❌ バックアップチェック失敗",
            "blocks": [
              {
                "type": "header",
                "text": {
                  "type": "plain_text",
                  "text": "❌ バックアップチェック失敗"
                }
              },
              {
                "type": "section",
                "text": {
                  "type": "mrkdwn",
                  "text": "*ワークフロー*: ${{ github.workflow }}\n*実行ID*: ${{ github.run_id }}\n\n<https://github.com/${{ github.repository }}/actions/runs/${{ github.run_id }}|詳細を確認>"
                }
              }
            ]
          }
      env:
        SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
        SLACK_WEBHOOK_TYPE: INCOMING_WEBHOOK
    ```

### 自動レポート生成

定期レポートを自動生成します。

=== "月次コストレポート"

    ```yaml title=".github/workflows/monthly-cost-report.yaml"
    name: Monthly Cost Report
    
    on:
      schedule:
        - cron: '0 10 1 * *'  # 毎月1日 10:00 JST
      workflow_dispatch:
    
    permissions:
      id-token: write
      contents: read
      issues: write
    
    jobs:
      cost-report:
        runs-on: ubuntu-latest
        steps:
          - name: Azure Login (OIDC)
            uses: azure/login@v1
            with:
              client-id: ${{ secrets.AZURE_CLIENT_ID }}
              tenant-id: ${{ secrets.AZURE_TENANT_ID }}
              subscription-id: ${{ secrets.AZURE_SUBSCRIPTION_ID }}
          
          - name: Get Cost Data
            run: |
              # 先月のコストを取得
              LAST_MONTH=$(date -d 'last month' '+%Y-%m-01')
              THIS_MONTH=$(date '+%Y-%m-01')
              
              az consumption usage list \
                --start-date $LAST_MONTH \
                --end-date $THIS_MONTH \
                --output json > cost-data.json
              
              # Subscription別に集計
              jq -r 'group_by(.subscriptionId) | 
                     map({subscription: .[0].subscriptionId, 
                          cost: map(.pretaxCost) | add}) | 
                     .[] | "\(.subscription),\(.cost)"' \
                cost-data.json > cost-by-sub.csv
          
          - name: Generate Report
            run: |
              cat << 'EOF' > report.md
              # 月次コストレポート
              
              **対象月**: $(date -d 'last month' '+%Y年%m月')
              **作成日**: $(date '+%Y-%m-%d')
              
              ## Subscription別コスト
              
              | Subscription | コスト (USD) |
              |-------------|-------------|
              EOF
              
              while IFS=, read -r sub cost; do
                echo "| $sub | \$$cost |" >> report.md
              done < cost-by-sub.csv
              
              cat << 'EOF' >> report.md
              
              ## アクション
              
              - コストが予算を超えているSubscriptionを確認
              - 未使用リソースの削除を検討
              - Reserved Instancesの活用を検討
              EOF
          
          - name: Create Issue
            uses: actions/github-script@v6
            with:
              script: |
                const fs = require('fs');
                const report = fs.readFileSync('report.md', 'utf8');
                
                await github.rest.issues.create({
                  owner: context.repo.owner,
                  repo: context.repo.repo,
                  title: `月次コストレポート - ${new Date().toLocaleDateString('ja-JP', {year: 'numeric', month: 'long'})}`,
                  body: report,
                  labels: ['report', 'cost']
                });
    ```

---

## Part 2: モニタリングとアラート

### Log Analyticsの活用

Log Analyticsでログを分析します。

=== "クエリ例"

    **ネットワークトラフィック分析**:
    
    ```kusto
    AzureDiagnostics
    | where Category == "AzureFirewallNetworkRule"
    | where TimeGenerated > ago(24h)
    | summarize count() by Action, msg_s
    | order by count_ desc
    ```
    
    **Policy違反の検出**:
    
    ```kusto
    AzureActivity
    | where OperationNameValue contains "Microsoft.Authorization/policyAssignments"
    | where ActivityStatusValue == "Failure"
    | project TimeGenerated, Caller, OperationNameValue, ActivityStatusValue
    | order by TimeGenerated desc
    ```
    
    **リソース作成トレンド**:
    
    ```kusto
    AzureActivity
    | where OperationNameValue contains "Microsoft.Resources/subscriptions/resourceGroups/write"
    | where ActivityStatusValue == "Success"
    | summarize count() by bin(TimeGenerated, 1d), SubscriptionId
    | render timechart
    ```

=== "Saved Queries"

    ```bash title="よく使うクエリを保存"
    az monitor log-analytics query \
      --workspace <workspace-id> \
      --analytics-query "$(cat network-traffic.kusto)" \
      --output table
    ```
    
    スクリプト化して定期実行できます。

### Azure Monitorの設定

Azure Monitorでメトリクスを監視します。

=== "Firewallメトリクス"

    **監視項目**:
    
    - Throughput（スループット）
    - SNAT port utilization（SNAT ポート使用率）
    - Firewall health state（正常性状態）
    - Application rules hit count（アプリケーションルールヒット数）
    
    ```bash title="メトリクス取得"
    az monitor metrics list \
      --resource <firewall-id> \
      --metric "Throughput" \
      --start-time 2026-01-20T00:00:00Z \
      --end-time 2026-01-20T23:59:59Z \
      --interval PT1H \
      --output table
    ```

=== "VNetメトリクス"

    **監視項目**:
    
    - BytesInDDoS（DDoS受信バイト数）
    - PacketsInDDoS（DDoS受信パケット数）
    - IfUnderDDoSAttack（DDoS攻撃検知）
    
    ```bash title="DDoS攻撃の確認"
    az monitor metrics list \
      --resource <vnet-id> \
      --metric "IfUnderDDoSAttack" \
      --start-time $(date -u -d '1 hour ago' '+%Y-%m-%dT%H:%M:%SZ') \
      --end-time $(date -u '+%Y-%m-%dT%H:%M:%SZ') \
      --interval PT5M
    ```

### Policy違反のアラート設定

Policy違反を検知してアラートします。

=== "アラートルール作成"

    ```bash title="Policy違反アラート"
    az monitor metrics alert create \
      --name "policy-violation-alert" \
      --resource-group alz-management-rg \
      --scopes "/providers/Microsoft.Management/managementGroups/alz" \
      --condition "count > 0" \
      --description "Policy違反を検知しました" \
      --evaluation-frequency 5m \
      --window-size 15m \
      --action <action-group-id>
    ```

=== "Action Group設定"

    ```bash title="Action Group作成"
    az monitor action-group create \
      --name "ops-team-alert" \
      --resource-group alz-management-rg \
      --short-name "OpsTeam" \
      --email-receiver \
        name="Admin" \
        email-address="admin@example.com" \
        use-common-alert-schema=true \
      --webhook-receiver \
        name="Slack" \
        service-uri="$SLACK_WEBHOOK_URL" \
        use-common-alert-schema=true
    ```

### コストアラートの設定

予算を超過した際にアラートします。

=== "予算設定"

    ```bash title="予算作成"
    az consumption budget create \
      --budget-name "monthly-budget-mgmt" \
      --category "Cost" \
      --amount 50000 \
      --time-grain "Monthly" \
      --start-date 2026-01-01 \
      --end-date 2026-12-31 \
      --resource-group alz-management-rg \
      --notifications \
        actual_GreaterThan_80_Percent='{
          "enabled": true,
          "operator": "GreaterThan",
          "threshold": 80,
          "contactEmails": ["admin@example.com"],
          "contactRoles": ["Owner"]
        }' \
        actual_GreaterThan_100_Percent='{
          "enabled": true,
          "operator": "GreaterThan",
          "threshold": 100,
          "contactEmails": ["admin@example.com"],
          "contactRoles": ["Owner"]
        }'
    ```

=== "コストアラートのレベル"

    | しきい値 | アクション |
    |---------|----------|
    | 50% | 情報通知（月次レポート） |
    | 80% | 警告通知（担当者確認） |
    | 90% | 緊急通知（即座に対応） |
    | 100% | 緊急通知（支出停止検討） |

### Dashboardの作成

監視ダッシュボードを作成します。

=== "Azure Portal Dashboard"

    **手順**:
    
    1. Azureポータル → Dashboard
    2. 「新しいダッシュボード」を作成
    3. タイルを追加：
       - Log Analyticsクエリ
       - メトリクスグラフ
       - Policy準拠状況
       - コスト推移
    
    **共有**:
    
    ```bash title="Dashboard共有"
    # Dashboardをエクスポート
    az portal dashboard show \
      --name "ALZ-Operations-Dashboard" \
      --resource-group alz-management-rg \
      --output json > dashboard.json
    
    # チームメンバーと共有
    az role assignment create \
      --assignee user@example.com \
      --role "Reader" \
      --scope "/subscriptions/<sub-id>/resourceGroups/alz-management-rg/providers/Microsoft.Portal/dashboards/ALZ-Operations-Dashboard"
    ```

=== "Grafana統合"

    **Azure Managed Grafana**:
    
    ```bash title="Grafana作成"
    az grafana create \
      --name alz-grafana \
      --resource-group alz-management-rg
    
    # Log Analyticsをデータソースに追加
    az grafana data-source create \
      --name alz-grafana \
      --definition '{
        "access": "proxy",
        "type": "grafana-azure-monitor-datasource",
        "jsonData": {
          "subscriptionId": "<sub-id>",
          "logAnalyticsDefaultWorkspace": "<workspace-id>"
        }
      }'
    ```

---

## Part 3: コスト最適化の実践

### コスト分析の方法

コストを分析します。

=== "Cost Analysisの活用"

    **手順**:
    
    1. Azureポータル → Cost Management + Billing
    2. Cost Analysis を開く
    3. スコープを選択（Management Group）
    4. グループ化：
       - Resource Group別
       - Service別
       - Location別
       - Tag別

=== "Azure CLI"

    ```bash title="コストデータ取得"
    # 今月のコスト
    az consumption usage list \
      --start-date $(date '+%Y-%m-01') \
      --end-date $(date '+%Y-%m-%d') \
      --output table
    
    # Service別集計
    az consumption usage list \
      --start-date $(date -d 'last month' '+%Y-%m-01') \
      --end-date $(date '+%Y-%m-01') \
      --output json | \
      jq -r 'group_by(.instanceName) | 
             map({service: .[0].instanceName, 
                  cost: map(.pretaxCost) | add}) | 
             sort_by(-.cost) | 
             .[] | "\(.service),\(.cost)"'
    ```

=== "Power BI統合"

    **Cost Management Connector**:
    
    1. Power BI Desktop を開く
    2. Get Data → Azure → Cost Management
    3. Management Groupスコープを指定
    4. レポート作成

### タグベースのコスト配賦

タグを使ってコストを配賦します。

=== "タグ戦略"

    **必須タグ**:
    
    - `Environment`: Dev / Test / Prod
    - `CostCenter`: 部署コード
    - `Application`: アプリケーション名
    - `Owner`: 責任者メールアドレス
    
    ```hcl title="terraform.tfvars"
    default_tags = {
      Environment = "Production"
      CostCenter  = "IT-001"
      Application = "ALZ"
      Owner       = "admin@example.com"
      ManagedBy   = "Terraform"
    }
    ```

=== "タグポリシー"

    ```json title="lib/policy_definitions/Require-Tags.json"
    {
      "name": "Require-Cost-Tags",
      "properties": {
        "displayName": "Require cost allocation tags",
        "policyType": "Custom",
        "mode": "Indexed",
        "policyRule": {
          "if": {
            "anyOf": [
              {
                "field": "tags['Environment']",
                "exists": "false"
              },
              {
                "field": "tags['CostCenter']",
                "exists": "false"
              }
            ]
          },
          "then": {
            "effect": "deny"
          }
        }
      }
    }
    ```

=== "コスト配賦レポート"

    ```bash title="タグ別コスト集計"
    az consumption usage list \
      --start-date $(date '+%Y-%m-01') \
      --end-date $(date '+%Y-%m-%d') \
      --output json | \
      jq -r '.[] | 
             select(.tags != null) | 
             {costCenter: .tags.CostCenter, 
              environment: .tags.Environment, 
              cost: .pretaxCost} | 
             "\(.costCenter),\(.environment),\(.cost)"' | \
      awk -F, '{arr[$1","$2]+=$3} END {for (i in arr) print i","arr[i]}'
    ```

### 未使用リソースの検出

未使用リソースを特定します。

=== "検出スクリプト"

    ```bash title="unused-resources.sh"
    #!/bin/bash
    
    echo "=== 未使用Public IP ==="
    az network public-ip list \
      --query "[?ipConfiguration==null].{Name:name, RG:resourceGroup, Location:location}" \
      --output table
    
    echo ""
    echo "=== 未接続Disk ==="
    az disk list \
      --query "[?diskState=='Unattached'].{Name:name, RG:resourceGroup, Size:diskSizeGb, Cost:sku.tier}" \
      --output table
    
    echo ""
    echo "=== 停止中VM（課金継続） ==="
    az vm list \
      --query "[?powerState=='VM deallocated'].{Name:name, RG:resourceGroup}" \
      --output table
    
    echo ""
    echo "=== 古いSnapshot（90日以上） ==="
    NINETY_DAYS_AGO=$(date -d '90 days ago' '+%Y-%m-%d')
    az snapshot list \
      --query "[?timeCreated<'$NINETY_DAYS_AGO'].{Name:name, RG:resourceGroup, Created:timeCreated}" \
      --output table
    ```

=== "自動削除ポリシー"

    ```yaml title=".github/workflows/cleanup-resources.yaml"
    name: Cleanup Unused Resources
    
    on:
      schedule:
        - cron: '0 11 * * 0'  # 毎週日曜 11:00 JST
      workflow_dispatch:
        inputs:
          dry_run:
            description: 'Dry run (true/false)'
            required: false
            default: 'true'
    
    jobs:
      cleanup:
        runs-on: ubuntu-latest
        steps:
          - name: Azure Login
            uses: azure/login@v1
            with:
              client-id: ${{ secrets.AZURE_CLIENT_ID }}
              tenant-id: ${{ secrets.AZURE_TENANT_ID }}
              subscription-id: ${{ secrets.AZURE_SUBSCRIPTION_ID }}
          
          - name: Delete Unused Public IPs
            run: |
              DRY_RUN="${{ github.event.inputs.dry_run || 'true' }}"
              
              az network public-ip list \
                --query "[?ipConfiguration==null].{Name:name, RG:resourceGroup}" \
                --output json > unused-ips.json
              
              if [ "$DRY_RUN" == "true" ]; then
                echo "Dry run - 削除対象:"
                cat unused-ips.json
              else
                jq -r '.[] | "\(.Name) \(.RG)"' unused-ips.json | \
                while read name rg; do
                  echo "Deleting: $name in $rg"
                  az network public-ip delete --name "$name" --resource-group "$rg"
                done
              fi
    ```

### Reserved Instancesの活用

Reserved Instancesでコスト削減します。

=== "推奨分析"

    **Azure Advisor**:
    
    ```bash title="RI推奨確認"
    az advisor recommendation list \
      --category "Cost" \
      --output table
    ```
    
    **対象ワークロード**:
    
    - ✅ 常時稼働VM（24/7）
    - ✅ 予測可能なワークロード
    - ✅ 1年以上継続利用
    
    **対象外**:
    
    - ❌ 開発・テスト環境（夜間停止）
    - ❌ スパイク的ワークロード
    - ❌ 短期プロジェクト

=== "購入計画"

    | VM Size | 台数 | 利用率 | RI推奨 | 節約額/年 |
    |---------|------|--------|--------|----------|
    | Standard_D2s_v3 | 10 | 100% | ✅ Yes | $2,500 |
    | Standard_D4s_v3 | 5 | 100% | ✅ Yes | $2,000 |
    | Standard_B2s | 3 | 60% | ❌ No | - |

### コスト削減施策

具体的なコスト削減施策です。

=== "即効性のある施策"

    **1. 未使用リソースの削除**:
    
    - Public IP: 削除で月額$3〜5/個
    - Unattached Disk: 削除でストレージコスト削減
    - 古いSnapshot: 削除でストレージコスト削減
    
    **2. VMサイズの最適化**:
    
    ```bash title="使用率の低いVM確認"
    az monitor metrics list \
      --resource <vm-id> \
      --metric "Percentage CPU" \
      --start-time $(date -u -d '7 days ago' '+%Y-%m-%dT%H:%M:%SZ') \
      --end-time $(date -u '+%Y-%m-%dT%H:%M:%SZ') \
      --interval PT1H \
      --aggregation Average
    ```
    
    CPU使用率が常に20%以下ならサイズダウン検討。
    
    **3. 開発環境の夜間停止**:
    
    ```bash title="夜間停止スケジュール"
    # Auto-shutdown設定
    az vm auto-shutdown \
      --resource-group dev-rg \
      --name dev-vm \
      --time 1900 \
      --timezone "Tokyo Standard Time"
    ```

=== "中長期施策"

    **1. Reserved Instances購入**:
    
    - 1年RI: 最大40%割引
    - 3年RI: 最大60%割引
    
    **2. Azure Hybrid Benefit**:
    
    - Windows Server: 既存ライセンス活用で最大40%削減
    - SQL Server: 最大55%削減
    
    ```bash title="Hybrid Benefit有効化"
    az vm update \
      --resource-group prod-rg \
      --name prod-vm \
      --license-type Windows_Server
    ```
    
    **3. ストレージ階層の最適化**:
    
    - Hot → Cool: 90日以上アクセスなし
    - Cool → Archive: 180日以上アクセスなし

---

## Part 4: 運用ベストプラクティス

### セキュリティ設定のレビュー

定期的にセキュリティをレビューします。

=== "Azure Security Center"

    **Secure Score確認**:
    
    ```bash title="Secure Score取得"
    az security secure-score-controls list \
      --output table
    ```
    
    **推奨事項の確認**:
    
    ```bash title="推奨事項一覧"
    az security assessment list \
      --query "[?status.code=='Unhealthy'].{Name:displayName, Severity:status.severity}" \
      --output table
    ```

=== "セキュリティレビューチェックリスト"

    **四半期ごとに確認**:
    
    - [ ] Secure Scoreが80点以上か
    - [ ] 重大な推奨事項がないか
    - [ ] 不要なPublic IPが公開されていないか
    - [ ] NSGルールが適切か
    - [ ] Firewallルールが最小権限か
    - [ ] 不要なユーザー権限がないか
    - [ ] MFAが全ユーザーで有効か
    - [ ] 監査ログが保持されているか

### IaCのベストプラクティス

Infrastructure as Codeのベストプラクティスです。

=== "コード品質"

    **必須事項**:
    
    - ✅ terraform fmt で整形
    - ✅ terraform validate でバリデーション
    - ✅ tflint でリンティング
    - ✅ コメントで意図を明確化
    - ✅ 変数のdescriptionを記載
    
    ```hcl title="良い例"
    variable "hub_address_space" {
      description = "Hub VNetのアドレス空間。/16推奨。"
      type        = list(string)
      default     = ["10.0.0.0/16"]
      
      validation {
        condition     = can(cidrhost(var.hub_address_space[0], 0))
        error_message = "有効なCIDR形式で指定してください。"
      }
    }
    ```

=== "State管理"

    **ベストプラクティス**:
    
    - ✅ Remote Backendを使用（Azure Storage）
    - ✅ State Lockingを有効化
    - ✅ バックアップを定期取得
    - ✅ 手動編集しない
    - ✅ チームでStateを共有
    
    ```bash title="Stateバックアップ"
    # 定期バックアップ
    terraform state pull > backup-$(date +%Y%m%d-%H%M%S).tfstate
    ```

=== "モジュール設計"

    **設計原則**:
    
    - 単一責任: 1モジュール1機能
    - 再利用可能: 汎用的に設計
    - テスト可能: 単体テスト実施
    - ドキュメント化: README.md必須
    
    ```text title="モジュール構造例"
    modules/
      spoke-vnet/
        main.tf          # リソース定義
        variables.tf     # 入力変数
        outputs.tf       # 出力値
        README.md        # ドキュメント
        examples/        # 使用例
          basic/
            main.tf
    ```

### チーム協働の改善

チームでの協働を改善します。

=== "ドキュメント文化"

    **必須ドキュメント**:
    
    - [ ] README.md（プロジェクト概要）
    - [ ] ARCHITECTURE.md（アーキテクチャ図）
    - [ ] OPERATIONS.md（運用手順）
    - [ ] TROUBLESHOOTING.md（トラブルシューティング）
    - [ ] CHANGELOG.md（変更履歴）
    
    ```markdown title="README.md テンプレート"
    # Azure Landing Zones
    
    ## 概要
    
    このリポジトリはAzure Landing Zonesの構成を管理します。
    
    ## アーキテクチャ
    
    [ARCHITECTURE.md](ARCHITECTURE.md)を参照してください。
    
    ## デプロイ手順
    
    [Chapter 14](docs-site/docs/14_Bootstrap_Phase_1.md)を参照してください。
    
    ## 運用手順
    
    [OPERATIONS.md](OPERATIONS.md)を参照してください。
    
    ## トラブルシューティング
    
    [TROUBLESHOOTING.md](TROUBLESHOOTING.md)を参照してください。
    ```

=== "コードレビュー基準"

    **レビューチェックリスト**:
    
    - [ ] コードが整形されているか（terraform fmt）
    - [ ] バリデーションが通るか（terraform validate）
    - [ ] Plan出力を確認したか
    - [ ] 意図しない変更がないか
    - [ ] ドキュメントが更新されているか
    - [ ] テストが通るか
    - [ ] セキュリティリスクがないか
    
    **レビューコメント例**:
    
    ```markdown
    ## レビューコメント
    
    ✅ Plan確認しました。問題ありません。
    
    ### 気になった点
    
    - Subnet名が `subnet1` となっているので、用途がわかる名前にしませんか？
      例: `app-subnet`, `data-subnet`
    
    ### 提案
    
    - Route Tableの設定も一緒にPRに含めた方が良いと思います。
    ```

=== "ナレッジ共有"

    **定期ミーティング**:
    
    - 週次: 運用レビュー会（30分）
      - 先週のインシデント
      - 今週の予定変更
      - 気になった点の共有
    
    - 月次: テクニカルディスカッション（1時間）
      - 新機能の紹介
      - ベストプラクティスの共有
      - 改善提案の議論
    
    **ナレッジベース**:
    
    - GitHub Wiki
    - Confluence
    - Notion
    
    よくある質問、トラブル事例、TIPSを蓄積。

### ドキュメントのメンテナンス

ドキュメントを最新に保ちます。

=== "更新タイミング"

    **即座に更新**:
    
    - アーキテクチャ変更時
    - 運用手順変更時
    - トラブル対応後（事例追加）
    
    **定期更新**:
    
    - 四半期ごと: 全体レビュー
    - 半期ごと: スクリーンショット更新
    - 年次: 大規模改訂

=== "ドキュメントレビュー"

    ```markdown title="ドキュメントレビューチェックリスト"
    ## ドキュメントレビュー - 2026 Q1
    
    ### README.md
    - [x] 最新のアーキテクチャ図
    - [x] デプロイ手順が正確
    - [ ] スクリーンショット更新（古い画面あり）
    
    ### OPERATIONS.md
    - [x] 運用手順が最新
    - [x] 新しいタスク追加済み
    - [x] 問題なし
    
    ### TROUBLESHOOTING.md
    - [x] 最近のトラブル事例追加
    - [ ] 古い情報の削除が必要
    
    ### アクション
    - スクリーンショットを更新する（担当: 山田）
    - 古い情報を削除する（担当: 佐藤）
    ```

### 定期レビューの実施

定期的にシステムをレビューします。

=== "月次レビュー"

    **チェック項目**:
    
    - [ ] Policy準拠状況（目標: 95%以上）
    - [ ] コスト状況（予算内か）
    - [ ] セキュリティスコア（目標: 80点以上）
    - [ ] インシデント件数（トレンド確認）
    - [ ] バックアップ成功率（目標: 100%）
    
    **レポート作成**:
    
    ```markdown title="月次レビューレポート"
    # 月次レビューレポート - 2026年1月
    
    ## サマリー
    
    - Policy準拠率: 97% ✅
    - コスト: 予算の85% ✅
    - セキュリティスコア: 83点 ✅
    - インシデント: 2件（前月比 -1）
    - バックアップ成功率: 100% ✅
    
    ## 詳細
    
    ### Policy準拠
    - 非準拠リソース: 5件
    - 主な原因: タグ未設定
    - アクション: タグポリシーを強化
    
    ### コスト
    - 総額: $42,500（予算: $50,000）
    - 削減施策: 未使用Public IP削除で$50/月削減
    
    ### インシデント
    1. Firewall SNAT枯渇（影響: 軽微）
       - 対応: ルール最適化
    2. VMバックアップ失敗（影響: なし）
       - 対応: エージェント再起動
    
    ## 次月アクション
    - [ ] Reserved Instances購入検討
    - [ ] セキュリティレビュー実施
    - [ ] ドキュメント更新
    ```

=== "四半期レビュー"

    **チェック項目**:
    
    - [ ] アーキテクチャの見直し
    - [ ] ポリシーの有効性評価
    - [ ] コスト最適化施策の効果測定
    - [ ] セキュリティ監査
    - [ ] DR訓練の実施
    - [ ] チームスキルの評価
    
    **改善計画**:
    
    ```markdown
    # 四半期改善計画 - 2026 Q2
    
    ## 目標
    
    1. セキュリティスコアを90点以上に向上
    2. コストを10%削減
    3. インシデント対応時間を50%短縮
    
    ## 施策
    
    ### セキュリティ向上
    - [ ] MFA全ユーザー適用（4月）
    - [ ] JIT Access導入（5月）
    - [ ] 脆弱性スキャン自動化（6月）
    
    ### コスト削減
    - [ ] RI購入（4月）
    - [ ] 未使用リソース自動削除（5月）
    - [ ] ストレージ階層最適化（6月）
    
    ### 運用効率化
    - [ ] Runbook整備（4-6月）
    - [ ] アラート自動対応（5-6月）
    - [ ] ダッシュボード改善（6月）
    ```

---

## まとめ

この章で学んだこと：

### ✅ Part 1: 運用自動化の設計

- 自動化すべきタスクの選定
- GitHub Actionsでの自動化
- Scheduled Workflowの活用
- Slack通知の統合
- 自動レポート生成

### ✅ Part 2: モニタリングとアラート

- Log Analyticsの活用
- Azure Monitorの設定
- Policy違反のアラート設定
- コストアラートの設定
- Dashboardの作成

### ✅ Part 3: コスト最適化の実践

- コスト分析の方法
- タグベースのコスト配賦
- 未使用リソースの検出
- Reserved Instancesの活用
- コスト削減施策

### ✅ Part 4: 運用ベストプラクティス

- セキュリティ設定のレビュー
- IaCのベストプラクティス
- チーム協働の改善
- ドキュメントのメンテナンス
- 定期レビューの実施

## 練習問題

理解度チェックです。休憩中に考えてみましょう。

### 問題1
Azure Monitorでアラートを設定する際、  
通知先として設定できるものは何ですか？（3つ答えてください）

### 問題2
コスト最適化のために確認すべきリソースは何ですか？

### 問題3
Scheduled Workflowを使って自動化できる運用タスクを2つ挙げてください。

---

## 練習問題の答え

### 答え1
1. **Email**（メール通知）
2. **Logic Apps**（カスタムワークフロー）
3. **Webhook**（Slack、Teams等への通知）

その他:
- **SMS**（テキストメッセージ）
- **Voice**（音声通話）
- **Azure Mobile App**（モバイル通知）

設定例:
```hcl
resource "azurerm_monitor_action_group" "alerts" {
  name                = "action-group-alerts"
  resource_group_name = azurerm_resource_group.management.name
  short_name          = "alerts"

  email_receiver {
    name          = "admin"
    email_address = "admin@example.com"
  }

  webhook_receiver {
    name        = "slack"
    service_uri = "https://hooks.slack.com/services/xxx"
  }
}
```

### 答え2

1. **使われていないリソース**
   - 削除し忘れたVM、Disk
   - 未割り当てのPublic IP
   - 未使用のStorage Account

2. **過剰スペックのリソース**
   - VM サイズ（Standard_D16s → Standard_D4s）
   - Log Analytics 保持期間（365日 → 90日）
   - Firewall SKU（Premium → Standard）

3. **予約インスタンスで割引可能なリソース**
   - 長期運用のVM
   - Log Analytics Commitment Tier

Azure Cost Management + Billingで確認:
```
Cost analysis → Resource → 最もコストがかかっているリソース
Advisor → Cost recommendations → コスト削減の推奨事項
```

### 答え3

1. **定期的なコンプライアンスチェック**
   ```yaml
   on:
     schedule:
       - cron: '0 9 * * 1'  # 毎週月曜 9時
   
   jobs:
     compliance:
       runs-on: ubuntu-latest
       steps:
         - name: Check Policy Compliance
           run: |
             az policy state list --resource-group rg-management-jp
   ```

2. **未使用リソースのレポート生成**
   ```yaml
   on:
     schedule:
       - cron: '0 0 1 * *'  # 毎月1日 0時
   
   jobs:
     unused-resources:
       runs-on: ubuntu-latest
       steps:
         - name: List unused resources
           run: |
             # 未割り当てのPublic IP
             az network public-ip list --query "[?ipConfiguration==null]"
             
             # 停止中のVM
             az vm list --query "[?powerState=='VM stopped']"
   ```

その他の例:
- State Lockの自動解除
- ログのアーカイブ
- セキュリティスキャン
- バックアップ検証

!!! success "教科書完了"
    お疲れ様でした！これでAzure Landing Zonesの構築から運用まで、すべてを学びました。
    
    実際のプロジェクトで活用してください。
