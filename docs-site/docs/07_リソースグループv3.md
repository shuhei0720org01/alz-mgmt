...
## Part 5: 実践 - 新しいリソースグループの追加

### シナリオ: セキュリティ用のリソースグループ追加
...
```hcl title="環境によって有効/無効"
connectivity_resource_groups = {
  optional_rg = {
    name     = "rg-optional-japaneast"
    location = "japaneast"
    settings = {
      enabled = false  # 無効化
    }
...
```text
Error: The resource group name must meet the following requirements:
- Between 1 and 90 characters long.
- Can only contain Alphanumerics, underscores, parentheses, hyphens, periods.
- Cannot end in a period
```
...
リソースグループの作成と公式モジュールの内部構造を理解しましたか？

次は[08_管理グループとポリシーv3.md](./08_管理グループとポリシーv3.md)に進んで、  
管理グループとポリシーの構築方法を学びましょう。

**所要時間**: 40分  
**難易度**: ★★★☆☆  
**次**: [08_管理グループとポリシーv3.md](./08_管理グループとポリシーv3.md)
