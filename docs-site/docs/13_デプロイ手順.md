# 13. デプロイ手順 - ゼロから環境を作る

## このChapterでやること

Azure Landing Zonesを実際にデプロイしよう。

**ゼロからスタート**：
```
何もない状態
  ↓
Azure準備
  ↓
GitHub準備
  ↓
デプロイ
  ↓
完成
```

**所要時間**：約2〜3時間

---

## 前提条件

### 必要なもの

#### 1. Azureアカウント

```
- Azure Subscription（Owner権限）
- Azure ADテナント
```

**確認**：
```bash
az login
az account show
```

#### 2. ツール

```
- Azure CLI：最新版
- Terraform：v1.12以上
- Git：最新版
- エディタ：VS Code推奨
```

**インストール確認**：
```bash title="必要ツールのバージョン確認"
az --version
terraform --version
git --version
code --version
```

#### 3. GitHubアカウント

```
- GitHubアカウント
- リポジトリ作成権限
```

---

## Phase 1: Azure準備

### Step 1-1: Subscriptionの確認

```bash title="AzureログインとSubscription選択"
# ログイン
az login

# Subscription一覧
az account list --output table

# 使うSubscriptionを選択
az account set --subscription "<SUBSCRIPTION_ID>"

# 確認
az account show
```

**重要**：
```
このSubscriptionに対してOwner権限が必要
  ↓
Management Group作成
リソース作成
RBACロール付与
  ↓
全部やるから
```

### Step 1-2: Service Principal作成（OIDC用）

**🔐 OIDC認証とは？（初心者向け説明）**

**従来の方法（パスワード認証）**:
```
GitHub → Azure
「パスワードは "abc123" です」
```

**問題点**:
- ❌ パスワードが漏れたら危険
- ❌ 定期的な更新が必要
- ❌ GitHubに保存するのが不安

**新しい方法（OIDC認証）**:
```
GitHub → Azure
「私はGitHubの公式リポジトリですよ（証明書提示）」
Azure → GitHub
「本当だ。じゃあ信じる」
```

**メリット**:
- ✅ パスワード不要（証明書ベース）
- ✅ 自動更新
- ✅ 漏洩リスクが低い

**Federated Credential（フェデレーテッド資格情報）とは？**:
「このGitHubリポジトリを信頼する」という設定。
特定のGitHubリポジトリからのリクエストだけを許可します。

```
Federated Credential = 「信頼するリポジトリのリスト」
```

**具体例**:
```
repo:shuhei0720org01/alz-mgmt:ref:refs/heads/main
 ↑         ↑             ↑          ↑
 |         |             |          mainブランチ
 |         |             リポジトリ
 |         オーナー
 プラットフォーム(GitHub)
```

これで、`shuhei0720org01/alz-mgmt` の `main` ブランチからだけアクセスできるようになります。

---

**では、実際に設定していきましょう**:

```bash title="OIDC用Service Principalの作成"
# 変数設定
APP_NAME="github-actions-alz-mgmt"
REPO_OWNER="your-github-username"
REPO_NAME="alz-mgmt"
SUBSCRIPTION_ID=$(az account show --query id -o tsv)

# Azure ADアプリ作成
az ad app create --display-name "$APP_NAME"

# App IDを取得
APP_ID=$(az ad app list --display-name "$APP_NAME" --query "[0].appId" -o tsv)
echo "APP_ID: $APP_ID"

# Service Principal作成
az ad sp create --id $APP_ID

# Owner権限付与（Subscription全体）
az role assignment create \
  --assignee $APP_ID \
  --role "Owner" \
  --scope "/subscriptions/$SUBSCRIPTION_ID"

# Federated Credential作成（mainブランチ用）
az ad app federated-credential create \
  --id $APP_ID \
  --parameters "{
    \"name\": \"github-actions-main\",
    \"issuer\": \"https://token.actions.githubusercontent.com\",
    \"subject\": \"repo:${REPO_OWNER}/${REPO_NAME}:ref:refs/heads/main\",
    \"audiences\": [\"api://AzureADTokenExchange\"]
  }"

# Federated Credential作成（PR用）
az ad app federated-credential create \
  --id $APP_ID \
  --parameters "{
    \"name\": \"github-actions-pr\",
    \"issuer\": \"https://token.actions.githubusercontent.com\",
    \"subject\": \"repo:${REPO_OWNER}/${REPO_NAME}:pull_request\",
    \"audiences\": [\"api://AzureADTokenExchange\"]
  }"
```

**保存しておく値**：
```bash title="GitHub Secretsに設定する値の表示"
echo "AZURE_CLIENT_ID: $APP_ID"
echo "AZURE_TENANT_ID: $(az account show --query tenantId -o tsv)"
echo "AZURE_SUBSCRIPTION_ID: $SUBSCRIPTION_ID"
```

**メモっておこう！** あとで使う。

### Step 1-3: Storage Account作成（Terraform State用）

```bash title="State用Storage Accountの作成"
# 変数設定
LOCATION="japaneast"
RG_NAME="rg-terraform-state"
STORAGE_ACCOUNT_NAME="stterraform$(openssl rand -hex 4)"  # ランダムな名前
CONTAINER_NAME="tfstate"

# Resource Group作成
az group create \
  --name $RG_NAME \
  --location $LOCATION

# Storage Account作成
az storage account create \
  --name $STORAGE_ACCOUNT_NAME \
  --resource-group $RG_NAME \
  --location $LOCATION \
  --sku Standard_LRS \
  --kind StorageV2 \
  --allow-blob-public-access false \
  --min-tls-version TLS1_2

# コンテナ作成
az storage container create \
  --name $CONTAINER_NAME \
  --account-name $STORAGE_ACCOUNT_NAME \
  --auth-mode login

# Service Principalに権限付与
STORAGE_ACCOUNT_ID=$(az storage account show \
  --name $STORAGE_ACCOUNT_NAME \
  --resource-group $RG_NAME \
  --query id -o tsv)

az role assignment create \
  --assignee $APP_ID \
  --role "Storage Blob Data Contributor" \
  --scope $STORAGE_ACCOUNT_ID
```

**保存しておく値**：
```bash title="Backend設定用の値表示"
echo "ARM_STORAGE_ACCOUNT_NAME: $STORAGE_ACCOUNT_NAME"
echo "ARM_CONTAINER_NAME: $CONTAINER_NAME"
echo "ARM_KEY: alz-mgmt.tfstate"
echo "ARM_RESOURCE_GROUP_NAME: $RG_NAME"
```

**これもメモっておこう！**

---

## Phase 2: GitHub準備

### Step 2-1: リポジトリ作成

```bash title="GitHubリポジトリのセットアップ"
# ローカルにclone済みの前提
cd ~/projects
git clone https://github.com/$REPO_OWNER/$REPO_NAME.git
cd $REPO_NAME

# または新規作成
mkdir alz-mgmt
cd alz-mgmt
git init
```

### Step 2-2: コードをコピー

このプロジェクトのファイルを全部コピー：

```bash
# 必要なファイル
├── .github/
│   └── workflows/
│       ├── ci.yaml
│       └── cd.yaml
├── lib/
│   ├── archetype_definitions/
│   └── architecture_definitions/
├── modules/
│   ├── config-templating/
│   ├── management_groups/
│   └── management_resources/
├── locals.tf
├── main.*.tf
├── outputs.tf
├── platform-landing-zone.auto.tfvars
├── terraform.tf
├── terraform.tfvars.json
└── variables.*.tf
```

**注意**：
```
.gitignore を作る：
```
```gitignore title=".gitignoreの設定"
# Terraform
.terraform/
*.tfstate
*.tfstate.*
*.tfvars
!platform-landing-zone.auto.tfvars
terraform.tfvars.json
.terraform.lock.hcl

# IDE
.vscode/
.idea/

# OS
.DS_Store
Thumbs.db
```
```

### Step 2-3: 設定ファイル編集

platform-landing-zone.auto.tfvars を自分の環境に合わせて編集：

```hcl title="最小構成の設定例"
# 最小限の設定例
starter_location_01 = "japaneast"
starter_location_02 = "japanwest"

root_id                     = "myorg"
root_name                   = "My Organization"
default_location            = "japaneast"
default_postfix             = "001"
subscription_id_management  = "<YOUR_MANAGEMENT_SUBSCRIPTION_ID>"
subscription_id_identity    = "<YOUR_IDENTITY_SUBSCRIPTION_ID>"
subscription_id_connectivity = "<YOUR_CONNECTIVITY_SUBSCRIPTION_ID>"

# 最小構成（コスト削減）
connectivity_type = "hub_and_spoke_vnet"

hub_and_spoke_networks_settings = {
  enabled_resources = {
    ddos_protection_plan = false  # ←無効化
  }
}

hub_virtual_networks = {
  primary = {
    location = "japaneast"
    
    enabled_resources = {
      firewall                              = false  # ←無効化
      bastion                               = false  # ←無効化
      virtual_network_gateway_express_route = false
      virtual_network_gateway_vpn           = false  # ←無効化
      private_dns_zones                     = true
      private_dns_resolver                  = false
    }
    
    hub_virtual_network = {
      name          = "vnet-myorg-jpe-hub"
      address_space = ["10.0.0.0/16"]
    }
  }
}
```

**Subscription ID確認**：
```bash title="Subscription一覧の表示"
az account list --output table
```

3つのSubscriptionが必要（同じでもOK）：

- Management：管理用
- Identity：ID管理用（AD DS等）
- Connectivity：ネットワーク用

### Step 2-4: Commit & Push

```bash title="初回コミットとPush"
git add .
git commit -m "Initial commit"
git push origin main
```

### Step 2-5: GitHub Secrets設定

```
GitHub → Settings → Secrets and variables → Actions → New repository secret
```

**設定するSecrets**：

```
AZURE_CLIENT_ID: <Step 1-2で保存した値>
AZURE_TENANT_ID: <Step 1-2で保存した値>
AZURE_SUBSCRIPTION_ID: <Step 1-2で保存した値>
ARM_STORAGE_ACCOUNT_NAME: <Step 1-3で保存した値>
ARM_CONTAINER_NAME: tfstate
ARM_KEY: alz-mgmt.tfstate
ARM_RESOURCE_GROUP_NAME: rg-terraform-state
```

**確認**：
```
GitHub → Settings → Secrets and variables → Actions
  ↓
7個のSecretsが表示されてる
```

### Step 2-6: Environment設定（推奨）

```
GitHub → Settings → Environments → New environment
  ↓
Name: production
  ↓
Configure environment
```

**設定**：
```
Environment protection rules:
- Required reviewers: 自分のアカウント追加
- Wait timer: 0 minutes（すぐ承認）
```

**効果**：
```
mainにマージ
  ↓
自動でapplyされない
  ↓
承認待ち
  ↓
承認
  ↓
apply実行
```

---

## Phase 3: ローカルでの初回デプロイ

**なぜローカルで？**
```
GitHub Actionsでいきなりやると：

- エラー時のデバッグが大変
- ログ見るのが面倒

ローカルで：

- すぐエラーわかる
- 設定ファイル調整しやすい
```

### Step 3-1: Terraform初期化

```bash title="Terraform初期化とBackend設定"
# 環境変数設定
export ARM_STORAGE_ACCOUNT_NAME="<Step 1-3の値>"
export ARM_CONTAINER_NAME="tfstate"
export ARM_KEY="alz-mgmt.tfstate"
export ARM_RESOURCE_GROUP_NAME="rg-terraform-state"

# Azureログイン（Owner権限のアカウント）
az login

# 使うSubscriptionを選択
az account set --subscription "<SUBSCRIPTION_ID>"

# Terraform初期化
terraform init

# Backend設定の確認
terraform init \
  -backend-config="storage_account_name=$ARM_STORAGE_ACCOUNT_NAME" \
  -backend-config="container_name=$ARM_CONTAINER_NAME" \
  -backend-config="key=$ARM_KEY" \
  -backend-config="resource_group_name=$ARM_RESOURCE_GROUP_NAME"
```

**成功すると**：
```
Terraform has been successfully initialized!
```

### Step 3-2: Terraform Validate

```bash title="構文チェック"
terraform validate
```

**成功すると**：
```
Success! The configuration is valid.
```

**エラーが出たら**：
```
Error: Missing required argument
  on main.tf line 10:
  10: resource "azurerm_resource_group" "example" {
```

設定ファイルを修正して再度validate。

### Step 3-3: Terraform Plan

```bash title="変更内容の確認"
terraform plan -out=tfplan
```

**初回は時間かかる**：5〜10分

**出力例**：
```
Plan: 45 to add, 0 to change, 0 to destroy.
```

**確認ポイント**：
```
- Management Group構造
- Resource Group
- Log Analytics Workspace
- VNet（無効化してれば作られない）
- Policy Assignments
```

**問題なければ次へ**。

### Step 3-4: Terraform Apply

```bash title="リソースの作成"
terraform apply tfplan
```

**所要時間**：20〜40分（初回）

**進行状況**：
```
module.management_groups[0].azurerm_management_group.level_1["landing-zones"]: Creating...
module.management_groups[0].azurerm_management_group.level_1["landing-zones"]: Creation complete after 15s
...
```

**完成！**
```
Apply complete! Resources: 45 added, 0 changed, 0 destroyed.
```

### Step 3-5: 確認

#### Management Group確認

```bash title="Management Groupの確認"
# Azure Portal
https://portal.azure.com/#view/Microsoft_Azure_ManagementGroups/ManagementGroupBrowseBlade/~/MGBrowse_overview

# CLI
az account management-group list --output table
```

**構造**：
```
myorg（Root）
├── platform
│   ├── management
│   ├── identity
│   └── connectivity
├── landing-zones
│   ├── corp
│   └── online
└── decommissioned
```

#### Resource Group確認

```bash title="Resource Group一覧の表示"
az group list --output table
```

**作られてるはず**：
```
rg-myorg-management-001
rg-myorg-connectivity-001
rg-myorg-identity-001
```

#### Log Analytics確認

```bash title="Log Analytics Workspaceの確認"
az monitor log-analytics workspace list --output table
```

---

## Phase 4: GitHub Actionsでのデプロイ

### Step 4-1: テスト用の変更

```bash title="テスト用リソースの追加"
# 新しいブランチ
git checkout -b feature/test-github-actions

# テスト用Resource Group追加
cat >> test.tf <<EOF
resource "azurerm_resource_group" "test_github_actions" {
  name     = "rg-test-github-actions"
  location = "japaneast"
}
EOF

# Commit & Push
git add test.tf
git commit -m "Test GitHub Actions"
git push origin feature/test-github-actions
```

### Step 4-2: Pull Request作成

```
GitHub → Pull requests → New pull request
  ↓
base: main ← compare: feature/test-github-actions
  ↓
Create pull request
```

### Step 4-3: CI確認

```
PR画面 → Checks タブ
  ↓
01 Azure Landing Zones Continuous Integration
  ↓
実行中...
```

**ログ確認**：
```
Actions → CI実行 → Details
  ↓
各ステップの進行状況
```

**成功すると**：
```
✓ Checkout
✓ Setup Terraform
✓ Azure Login
✓ Terraform Init
✓ Terraform Validate
✓ Terraform Plan
✓ Comment PR

Plan: 1 to add, 0 to change, 0 to destroy.
```

**PR画面にコメント**：
```
## Terraform Plan
...
+ azurerm_resource_group.test_github_actions
    name     = "rg-test-github-actions"
    location = "japaneast"
```

### Step 4-4: 承認 & マージ

```
PR画面 → Files changed → Review changes → Approve
  ↓
Merge pull request
  ↓
Confirm merge
```

### Step 4-5: CD確認

```
Actions → 02 Azure Landing Zones Continuous Delivery
  ↓
実行中...
```

**Environment承認設定してる場合**：
```
実行中 → Waiting for approval
  ↓
Review deployments → Approve
  ↓
実行再開
```

**完了**：
```
✓ Checkout
✓ Setup Terraform
✓ Azure Login
✓ Terraform Init
✓ Terraform Apply

Apply complete! Resources: 1 added, 0 changed, 0 destroyed.
```

### Step 4-6: Azure確認

```bash title="作成されたリソースの確認"
az group show --name rg-test-github-actions
```

**作られてる！**

### Step 4-7: クリーンアップ

```bash title="テストリソースの削除"
# ブランチ作成
git checkout main
git pull
git checkout -b feature/cleanup-test

# test.tf削除
git rm test.tf
git commit -m "Cleanup test resource group"
git push origin feature/cleanup-test

# PR作成 → CI → 承認 → マージ → CD
# rg-test-github-actions が削除される
```

---

## Phase 5: 本格的なデプロイ

### Step 5-1: 設定ファイルを本番仕様に

```bash title="本番環境用ブランチ作成"
git checkout -b feature/enable-production-resources
```

platform-landing-zone.auto.tfvars 編集：

```hcl title="本番環境の設定（Firewall有効化）"
# Firewall有効化
hub_virtual_networks = {
  primary = {
    location = "japaneast"
    
    enabled_resources = {
      firewall                              = true   # ←有効化
      bastion                               = true   # ←有効化
      virtual_network_gateway_express_route = false
      virtual_network_gateway_vpn           = false  # ←必要なら有効化
      private_dns_zones                     = true
      private_dns_resolver                  = false
    }
    
    hub_virtual_network = {
      name          = "vnet-myorg-jpe-hub"
      address_space = ["10.0.0.0/16"]
      routing_address_space         = ["10.0.0.0/22"]
      route_table_name_firewall     = "rt-myorg-jpe-firewall"
      route_table_name_user_subnets = "rt-myorg-jpe-user"
    }
    
    firewall = {
      subnet_address_prefix            = "10.0.0.0/26"
      management_subnet_address_prefix = "10.0.0.64/26"
      name                             = "fw-myorg-jpe-hub"
      
      default_ip_configuration = {
        public_ip_config = {
          name = "pip-fw-myorg-jpe-hub"
        }
      }
      
      management_ip_enabled = true
      management_ip_configuration = {
        public_ip_config = {
          name = "pip-fw-myorg-jpe-hub-mgmt"
        }
      }
    }
    
    firewall_policy = {
      name = "fwp-myorg-jpe-hub"
    }
    
    bastion = {
      subnet_address_prefix = "10.0.2.0/27"
      name                  = "bas-myorg-jpe-hub"
      
      public_ip = {
        name  = "pip-bas-myorg-jpe-hub"
        zones = []  # Japan region
      }
    }
  }
}
```

### Step 5-2: Commit & PR

```bash title="本番設定のコミット"
git add platform-landing-zone.auto.tfvars
git commit -m "Enable Firewall and Bastion for production"
git push origin feature/enable-production-resources

# PR作成
```

### Step 5-3: Plan確認

```
PR画面 → Checks → Details
  ↓
Plan結果確認
```

**追加されるリソース**：
```
+ Azure Firewall
+ Firewall Policy
+ Azure Bastion
+ Public IP x3
+ Route Tables
+ Subnets
```

**コスト確認**：
```
Firewall: 約17万円/月
Bastion: 約2.7万円/月
-----------------
合計: 約20万円/月
```

**OK？** → 承認 & マージ

### Step 5-4: デプロイ待機

```
CD実行 → 30〜60分かかる
```

**Firewallの作成が遅い**。

コーヒー飲んで待とう☕

---

## Phase 6: 動作確認

### Step 6-1: Management Group確認

```bash title="Management Group階層構造の確認"
az account management-group show \
  --name myorg \
  --expand \
  --recurse
```

階層構造が表示される。

### Step 6-2: Policy確認

```bash title="Policy Assignmentの確認"
# Policy Assignment一覧
az policy assignment list --output table

# 特定のManagement Groupに割り当てられたPolicy
az policy assignment list \
  --scope "/providers/Microsoft.Management/managementGroups/myorg-landing-zones" \
  --output table
```

### Step 6-3: Log Analytics確認

```bash title="Log Analyticsデータの確認"
# Workspace確認
WORKSPACE_ID=$(az monitor log-analytics workspace show \
  --resource-group rg-myorg-management-001 \
  --workspace-name law-myorg-management-001 \
  --query id -o tsv)

# データ確認（サンプルクエリ）
az monitor log-analytics query \
  --workspace $WORKSPACE_ID \
  --analytics-query "AzureActivity | take 10" \
  --output table
```

### Step 6-4: Firewall確認

```bash title="Azure Firewallの確認"
# Firewall確認
az network firewall show \
  --resource-group rg-myorg-connectivity-001 \
  --name fw-myorg-jpe-hub

# Public IP確認
az network firewall show \
  --resource-group rg-myorg-connectivity-001 \
  --name fw-myorg-jpe-hub \
  --query "ipConfigurations[0].publicIpAddress.id" -o tsv
```

### Step 6-5: Bastion確認

```bash title="Azure Bastionの確認"
az network bastion show \
  --resource-group rg-myorg-connectivity-001 \
  --name bas-myorg-jpe-hub
```

### Step 6-6: Spoke VNet追加テスト

```bash title="Spoke VNet用ブランチ作成"
git checkout -b feature/add-spoke-vnet
```

**新規ファイル作成**：`spoke.tf`

```hcl title="Spoke VNetとHubへのピアリング設定"
resource "azurerm_virtual_network" "spoke1" {
  name                = "vnet-myorg-spoke1"
  location            = "japaneast"
  resource_group_name = azurerm_resource_group.spoke1.name
  address_space       = ["10.1.0.0/16"]
}

resource "azurerm_resource_group" "spoke1" {
  name     = "rg-myorg-spoke1"
  location = "japaneast"
}

# Hub VNetとピアリング
resource "azurerm_virtual_network_peering" "spoke1_to_hub" {
  name                      = "peer-spoke1-to-hub"
  resource_group_name       = azurerm_resource_group.spoke1.name
  virtual_network_name      = azurerm_virtual_network.spoke1.name
  remote_virtual_network_id = module.hub_and_spoke_vnet[0].hub_virtual_networks["primary"].virtual_network.resource_id
  
  allow_virtual_network_access = true
  allow_forwarded_traffic      = true
  allow_gateway_transit        = false
  use_remote_gateways          = false
}

resource "azurerm_virtual_network_peering" "hub_to_spoke1" {
  name                      = "peer-hub-to-spoke1"
  resource_group_name       = "rg-myorg-connectivity-001"
  virtual_network_name      = "vnet-myorg-jpe-hub"
  remote_virtual_network_id = azurerm_virtual_network.spoke1.id
  
  allow_virtual_network_access = true
  allow_forwarded_traffic      = true
  allow_gateway_transit        = true
  use_remote_gateways          = false
  
  depends_on = [module.hub_and_spoke_vnet]
}
```

```bash
git add spoke.tf
git commit -m "Add spoke VNet"
git push origin feature/add-spoke-vnet

# PR → CI → 承認 → マージ → CD
```

**Spoke VNetが作られる！**

---

## トラブルシューティング

### エラー1: OIDC認証失敗

```
Error: Unable to get ACTIONS_ID_TOKEN_REQUEST_URL
```

**原因**：Federated Credentialの設定間違い

**確認**：
```bash title="Federated Credentialの確認"
az ad app federated-credential list --id $APP_ID
```

**subject確認**：
```json
{
  "subject": "repo:your-github-username/alz-mgmt:ref:refs/heads/main"
}
```

**間違ってたら削除して再作成**：
```bash title="Federated Credentialの再作成"
az ad app federated-credential delete \
  --id $APP_ID \
  --federated-credential-id <CREDENTIAL_ID>

# 再作成（Phase 2のコマンド）
```

### エラー2: Storage Account権限エラー

```
Error: Failed to get existing workspaces: blob not found
```

**原因**：Service PrincipalにStorage権限がない

**対処法**：
```bash title="Storage権限の付与"
STORAGE_ACCOUNT_ID=$(az storage account show \
  --name $ARM_STORAGE_ACCOUNT_NAME \
  --resource-group $ARM_RESOURCE_GROUP_NAME \
  --query id -o tsv)

az role assignment create \
  --assignee $APP_ID \
  --role "Storage Blob Data Contributor" \
  --scope $STORAGE_ACCOUNT_ID
```

### エラー3: Subscription権限エラー

```
Error: insufficient privileges to complete the operation
```

**原因**：Service PrincipalにOwner権限がない

**確認**：
```bash title="Subscription権限の確認"
az role assignment list \
  --assignee $APP_ID \
  --scope "/subscriptions/$SUBSCRIPTION_ID"
```

**Owner権限付与**：
```bash title="Service PrincipalへのOwner権限付与"
az role assignment create \
  --assignee $APP_ID \
  --role "Owner" \
  --scope "/subscriptions/$SUBSCRIPTION_ID"
```

### エラー4: Terraform Lock

```
Error: Error acquiring the state lock
```

**原因**：前回のワークフローが失敗してLockが残ってる

**対処法**：
```bash title="State Lockファイルの削除"
# Azure Portal → Storage Account → tfstateコンテナ
# .terraform.lock.info ファイルを削除

# またはCLI
az storage blob delete \
  --account-name $ARM_STORAGE_ACCOUNT_NAME \
  --container-name $ARM_CONTAINER_NAME \
  --name .terraform.lock.info \
  --auth-mode login
```

---

## デプロイ後の確認チェックリスト

### ✅ Management Group

```bash title="Management Group階層構造の確認"
# 階層構造確認
az account management-group list --output table

# 期待される構造：
# myorg
# ├── myorg-platform
# │   ├── myorg-management
# │   ├── myorg-identity
# │   └── myorg-connectivity
# ├── myorg-landing-zones
# │   ├── myorg-corp
# │   └── myorg-online
# └── myorg-decommissioned
```

### ✅ Resource Group

```bash title="Resource Groupの確認"
az group list --query "[].{Name:name, Location:location}" --output table

# 期待されるResource Group：
# rg-myorg-management-001
# rg-myorg-connectivity-001
# rg-myorg-identity-001
# rg-terraform-state
```

### ✅ Log Analytics Workspace

```bash title="Log Analytics Workspaceの確認"
az monitor log-analytics workspace list --output table

# law-myorg-management-001 が存在
```

### ✅ VNet（有効化している場合）

```bash title="VNetの確認"
az network vnet list --output table

# vnet-myorg-jpe-hub が存在
```

### ✅ Firewall（有効化している場合）

```bash title="Firewallの確認"
az network firewall list --output table

# fw-myorg-jpe-hub が存在
```

### ✅ Policy Assignment

```bash title="Policy Assignmentの確認"
az policy assignment list \
  --scope "/providers/Microsoft.Management/managementGroups/myorg" \
  --output table

# 複数のPolicy Assignmentが表示される
```

### ✅ GitHub Actions履歴

```
GitHub → Actions
  ↓
CI/CDの実行履歴が残ってる
```

---

## まとめ

**デプロイの流れ**：
```
Phase 1: Azure準備（Service Principal、Storage Account）
Phase 2: GitHub準備（リポジトリ、Secrets）
Phase 3: ローカルでの初回デプロイ
Phase 4: GitHub Actionsでのデプロイ
Phase 5: 本格的なデプロイ
Phase 6: 動作確認
```

**所要時間**：
```
Phase 1: 30分
Phase 2: 30分
Phase 3: 60分
Phase 4: 30分
Phase 5: 60分
Phase 6: 20分
-----------
合計: 約3.5時間
```

**重要ポイント**：

- Service Principal: OIDC設定必須
- Storage Account: State保存先
- ローカル実行: 最初はローカルで確認
- 段階的デプロイ: いきなり全部有効化しない
- コスト確認: Firewall等を有効化する前に確認

---

## 練習問題

実践的な理解度チェックです。

### 問題1
OIDC認証とパスワード認証の違いを3つ挙げてください。

### 問題2
以下のエラーが出た場合、何が原因ですか？
```
Error: zones are not supported in this region
```

### 問題3
Terraform Stateファイルをローカルではなく、  
Azure Storage Accountに保存する理由を2つ挙げてください。

---

## 練習問題の答え

### 答え1
**OIDC認証とパスワード認証の違い**:

1. **セキュリティ**: OIDC = パスワード不要、パスワード認証 = 漏洩リスクあり
2. **更新**: OIDC = 自動更新、パスワード認証 = 定期的な手動更新が必要
3. **信頼モデル**: OIDC = 証明書ベース、パスワード認証 = 秘密情報の保存

### 答え2
**原因**: Japan East等のリージョンで、Availability Zonesに対応していないリソースに対して  
`zones = ["1", "2", "3"]`のように指定している。

**解決策**: `zones = []`（空リスト）を指定する。

```hcl
primary_firewall_client_public_ip_zones = []
```

### 答え3
**Terraform Stateをリモート保存する理由**:

1. **チーム共有**: 複数人で同じStateを参照できる。ローカルだと個人のPCにしか保存されない。
2. **安全性**: ローカルPCの故障やファイル削除でStateが失われるリスクを回避。Azure Storageはバックアップされている。

**追加の利点**:
- State Lockingでチーム内の競合を防げる
- GitHub Actionsから直接アクセスできる

---

次のChapterでは、よくあるトラブルとその対処法を見ていきます。
デプロイ中にハマりやすいポイントを解説するよ。

---

**所要時間**: 65分  
**難易度**: ★★★★★  
**前**: [12_GitHub_Actions.md](./12_GitHub_Actions.md)  
**次**: [14_トラブルシューティング.md](./14_トラブルシューティング.md)
