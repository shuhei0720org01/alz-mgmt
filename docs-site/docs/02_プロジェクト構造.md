...
```hcl title="platform-landing-zone.auto.tfvars（冒頭部分）"
/*
...
count = var.bootstrap_settings.enabled ? 1 : 0
```
- `enabled`が`true`なら1個作る（module作る）
- `false`なら0個（module作らない）
...
configuration_file_path = "${path.root}/platform-landing-zone.auto.tfvars"
```
`path.root`= プロジェクトのルートディレクトリ。  
どこからコマンド実行しても同じパスになる。
...
`hub_and_spoke_vnet`が無効（count=0）なら、エラーじゃなくて`null`を返す。
...
**inputs**: ユーザーが選ぶ（applyかdestroyか）  
**uses**: 別リポジトリのワークフローを使う
