# 06. 設定テンプレート - config-templatingモジュール

## このChapterでやること

`modules/config-templating/`モジュールの仕組みを理解しよう。

**例えるなら**：

- **Mad Libs**（穴埋めゲーム）みたいなやつ
- テンプレート：「今日の天気は`{{weather}}`だよ」
- 変数：`weather = "晴れ"`
- 結果：「今日の天気は晴れだよ」

このモジュールは、設定ファイル内の`{{xxx}}`を実際の値に置き換えてくれるんだ。

---

## テンプレート処理の全体フロー

config-templatingモジュールの処理の流れを視覚化してみましょう。

```mermaid
graph TB
    A[入力: platform-landing-zone.auto.tfvars] --> B[variables.tf<br/>入力変数の受け取り]
    
    B --> C[data.tf<br/>Azureリージョン情報取得]
    B --> D[locals.config.tf<br/>テンプレート処理]
    
    C --> C1[regions モジュール<br/>geo_code, short_name]
    C --> C2[azurerm_client_config<br/>tenant_id等]
    
    D --> D1[ステップ1: リージョンマッピング<br/>starter_locations_short]
    D1 --> D2[ステップ2: 組み込み置換作成<br/>built_in_replacements]
    D2 --> D3[ステップ3: カスタム置換処理<br/>custom_replacements]
    D3 --> D4[ステップ4: テンプレート適用<br/>templatestring]
    
    D4 --> E[outputs.tf<br/>処理済み設定を出力]
...
1. **入力受け取り**：tfvarsファイルから設定を取得
2. **データ取得**：Azureリージョン情報を取得
3. **テンプレート処理**：`{{xxx}}`を実際の値に置換
4. **結果出力**：置換完了した設定を返す
...
**流れ**：
1. `variables.tf`：入力を受け取る
2. `data.tf`：Azureリージョン情報を取得
3. `locals.config.tf`：テンプレートを処理
4. `outputs.tf`：結果を出力

---

## Part 1: 入力変数（variables.tf）
...
Chapter 3で見た`custom_replacements`をそのまま受け取るんだ。
...
この`inputs`の中身がテンプレート処理される。

---

## Part 2: データソース（data.tf）
...
**何をしているのでしょうか？**
Azureの全リージョン情報を取得しています。

**取得できる情報**：

- `geo_code`：地理コード（例：`jp`、`us`）
- `short_name`：短縮名（例：`jpe`、`jpw`）
...
## Part 3: テンプレート処理（locals.config.tf）
...
**優先順位**：
1. **ユーザーの上書き**：手動で指定した値
2. **geo_code**：`jp`、`us`みたいな地理コード
3. **short_name**：`jpe`、`jpw`みたいな短縮名
4. **フルネーム**：`japaneast`（最後の手段）
...
**何これ？**
めちゃくちゃ複雑だけど、やってることは単純。

**問題**：
`templatestring()`関数は文字列しか受け取れない。
でも、`var.custom_replacements.names`はオブジェクト。

**解決策**：
1. **JSON化**：オブジェクト → JSON文字列
2. **テンプレート処理**：`{{xxx}}`を置換
3. **オブジェクトに戻す**：JSON文字列 → オブジェクト

#### 詳細な流れ

**🤔 その前に: なぜ jsonencode/jsondecode を使うのか？**

**問題**: Terraformのオブジェクトをそのまま文字列操作できない
...
**解決策**: オブジェクト → JSON文字列 → 文字列操作 → オブジェクトに戻す

```hcl
# ステップ1: オブジェクト → JSON文字列に変換
...
**なんで保護？**
`templatestring()`が`true`/`false`を文字列`"true"`/`"false"`に変換してしまうためです。
それを防ぐために一時的に`{{string_true}}`に変えてる。
...
**やってること**：
`{{string_true}}`を`true`に戻す（元のブール値に）。
...
組み込み置換 + カスタム名を1つにまとめる。

---

### ステップ3: リソースグループとリソースIDの処理
...
**やってること**：
`custom_names`と同じ処理を`resource_group_identifiers`に対して実行しています。

**違い**：
`local.custom_name_replacements`を使ってテンプレート処理。
つまり、リソースグループIDの中でリソース名を参照できる。
...
`{{resource_group_name}}`が`rg-jp-hub`に置き換わってる。わかる？
...
**やってること**：
同じ処理を`resource_identifiers`に対して実行。

`local.custom_resource_group_replacements`を使うから、**リソースIDの中でリソースグループIDを参照できる**。
...
**やってること**：
`var.inputs`（全設定）に対して、今まで作った置換ルールを適用しています。
...
**流れ**：
1. **JSON化**：各設定をJSON文字列に
2. **テンプレート処理**：`{{xxx}}`を置換
3. **オブジェクトに戻す**：JSON → オブジェクト
...
### 1. 組み込み置換の作成
```
starter_locations → starter_locations_short → built_in_replacements
```

**結果**：

- `starter_location_01`、`starter_location_01_short`
- `subscription_id_xxx`
...
- **段階的に処理**：名前 → リソースグループID → リソースID
- **前のステップの結果を次で使う**：名前がRG IDの中で使える
...
`{{string_true}}`のトリックで、ブール値が保護されてるんだ。
...
後のステップで前のステップの結果を使える。

---

## まとめ

**config-templatingモジュールの役割**：
1. **リージョン情報の正規化**：`japaneast` → `jp`
2. **組み込み置換の作成**：リージョン、サブスクリプションID
3. **カスタム置換の段階的処理**：名前 → RG ID → リソースID
4. **テンプレート処理**：`{{xxx}}`を実際の値に置換
5. **ブール値の保護**：`true`/`false`が文字列にならないように

**覚えておくこと**：

- `templatestring()`：`{{xxx}}`を置換
- `jsonencode()`/`jsondecode()`：オブジェクト ⇔ JSON文字列
- `coalesce()`：最初のnullじゃない値
- `try()`：エラーをキャッチ

次のChapterでは、リソースグループの作成を見ていきます。
実際にAzureリソースを作る最初のステップです。
...
### 問題1
`starter_location_01 = "japaneast"`のとき、  
`starter_location_01_short`の値は何になりますか？

### 問題2
次のテンプレート文字列で、  
`{{starter_location_01}}`が`japaneast`に置換される処理の名前は何ですか？
...
### 問題3
`jsonencode()`と`jsondecode()`の使い分けを説明してください。
...
`locals.config.tf`の`starter_locations_short`マップで、  
`japaneast` → `jp`に変換されます。

### 答え2
**`templatestring()`関数**です。
...
`{{xxx}}`を実際の値に置換する処理を「テンプレート処理」と呼びます。

### 答え3
- **`jsonencode()`**：オブジェクト → JSON文字列（文字列化）
- **`jsondecode()`**：JSON文字列 → オブジェクト（オブジェクト化）

```hcl
# オブジェクトを文字列にする
str = jsonencode({ name = "test" })  # '{"name":"test"}'

# 文字列をオブジェクトに戻す
obj = jsondecode(str)  # { name = "test" }
```

テンプレート処理では、文字列にしてから置換し、最後にオブジェクトに戻します。

---

**所要時間**: 50分  
**難易度**: ★★★★★  
**前**: [05_ローカル変数.md](./05_ローカル変数.md)  
