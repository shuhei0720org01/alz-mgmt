# 12. 管理リソース - Log AnalyticsとDCRの完全解説

!!! info "この章で学ぶこと 📚"
    管理リソース（Log Analytics、Data Collection Rules、Managed Identity）の実装を完全に理解します：🛠️

    1. 実際の設定ファイル（platform-landing-zone.auto.tfvars）の構造🗂️
...
まず、管理リソースの構成を図で見てみよう：

![管理リソース構成](./img/diagrams/management-resources.svg)

---

## Part 1: 設定ファイル（platform-landing-zone.auto.tfvars）🗂️

ここからも実際のファイルを見ながらいこう。👀

### management_resource_settings⚙️
...
## Part 2: main.management.tfとモジュール構造🏗️
...
受け取った設定値を公式モジュールに渡すための変数定義。実際にプロジェクトで使うのは以下の7つの設定だけで十分：

- `location`：リソースのデプロイ先リージョン（必須）
- `log_analytics_workspace_name`：Log Analyticsワークスペース名（省略時は自動生成）
- `resource_group_name`：リソースグループ名（省略時は自動生成）
- `data_collection_rules`：3種類のデータ収集ルール設定
- `user_assigned_managed_identities`：Azure Monitor Agent用のManaged Identity設定

Log Analyticsの詳細設定（`log_analytics_workspace_*`）やSentinel設定は、省略してもデフォルト値で動作する。
...
**所要時間**: 60分  
**難易度**: ★★★★☆  
**前**: [11_管理グループとポリシー.md](./11_管理グループとポリシー.md)  
**次**: [13_Hub-and-Spoke.md](./13_Hub-and-Spoke.md)
