...
**対処法**：
```bash title="Service Principal権限の確認と付与"
# 現在の権限確認
...
**対処法1（推奨）**：ロック解除
```bash title="Terraform State Lockの解除"
# ロックID確認（エラーメッセージから）
...
**確認**：
```bash title="Storage AccountとContainerの確認"
# Storage Account確認
...
**対処法**：
```bash title="Storage Containerの作成"
...
**対処法**：
```bash title="Storage Blob Data Contributor権限の付与"
...
**対処法**：
```hcl title="Availability Zonesの無効化"
# NG
bastion = {
  zones = ["1", "2", "3"]
}
...
**例（NG）**：
```hcl title="Management Group名が長すぎる例"
...
**対処法**：
```yaml title="GitHub Repository設定パス"
...
**対処法1（Concurrency設定）**：
```yaml title="cd.yaml - Concurrency設定"
...
**対処法2（手動でLock解除）**：
```bash title="Terraform State Lockの強制解除"
terraform force-unlock <LOCK_ID>
```
...
```bash title="Terraformデバッグログの有効化"
# デバッグモード有効化
export TF_LOG=DEBUG
...
# 2. terraform.tf更新
# terraform.tf
```hcl title="terraform.tf - バージョン更新"
terraform {
...
# .github/workflows/ci.yaml
```yaml title="ci.yaml - Terraformバージョン更新"
...
**回答**：
```bash title="Terraform管理からリソースを外す手順"
# 1. Stateから削除
...
**回答**：
```bash title="既存リソースをTerraformにインポート"
# 1. .tfファイルに定義追加
# main.tf
