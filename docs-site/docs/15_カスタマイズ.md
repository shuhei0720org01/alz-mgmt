...
**内容**：
```yaml title="root_custom.alz_archetype_override.yaml"
base_archetype: root
name: root_custom
...
```yaml title="Security Management Groupの親変更"
- id: security
...
```hcl title="Application Gateway用サブネットの追加"
...
```hcl title="サブスクリプションをManagement Groupに関連付け"
...
```hcl title="Log Analytics保持期間の延長"
...
```hcl title="カスタムData Collection Ruleの作成"
