...
```hcl title="modules/config-templating/terraform.tf（完全版）"
terraform {
...
```hcl title="modules/config-templating/data.tf（完全版：8行）"
module "regions" {
...
| ファイル | 行数 | 役割 |
|---------|------|------|
| terraform.tf | 8行 | バージョン設定 |
| variables.tf | 59行 | 入力変数定義 |
| data.tf | 8行 | データ取得 |
| locals.config.tf | 75行 | 置換処理 |
| outputs.tf | 6行 | 結果出力 |
| **合計** | **156行** | **全体** |
