# 08. 管理グループとポリシー - ガバナンスの要

## 8つの設計領域との対応

このChapterは以下の設計領域を実装します：

- ✅ **2. Identity and Access Management**（ID・アクセス管理 - RBAC、権限管理）
- ✅ **3. Resource Organization**（リソース整理 - 管理グループ階層）
- ✅ **5. Security**（セキュリティ - セキュリティポリシー）
- ✅ **7. Governance**（ガバナンス - ポリシー定義と適用）
...
管理グループとポリシーの仕組みを理解しよう。

**管理グループって何？**
サブスクリプションを束ねる「フォルダ」みたいなもん。

**ポリシーって何？**
「これやっちゃダメ」「これやらないとダメ」みたいなルール。

**例えるなら**：

- **管理グループ**：会社の部署（営業部、開発部、総務部）
- **ポリシー**：社内規定（「経費は1万円まで」「セキュリティ研修必須」）
- **サブスクリプション**：各チーム
...
| 管理グループ | 役割 | 誰が使う | ポリシー |
|-------------|------|---------|--------|
| **platform** | インフラ基盤 | インフラチーム | 厳しい（変更制限） |
| - management | ログ・監視 | インフラチーム | 削除禁止 |
| - connectivity | ネットワーク | ネットワークチーム | 変更要承認 |
| - identity | 認証・認可 | セキュリティチーム | 最高レベル |
| - security | セキュリティ監視 | セキュリティチーム | 最高レベル |
| **landingzones** | アプリケーション | 開発チーム | 標準 |
| - corp | 社内アプリ | 開発チーム | 標準+社内接続のみ |
| - online | 公開アプリ | 開発チーム | 標準+インターネット許可 |
...
## Part 1: アーキテクチャ定義（alz_custom.alz_architecture_definition.yaml）
...
## Part 2: アーキタイプ定義（archetype_definitions/）
...
- `root_custom`：ルート用のポリシーセット
- `platform_custom`：プラットフォーム用
- `landing_zones_custom`：ランディングゾーン用
...
`lib/architecture_definitions/alz_custom.alz_architecture_definition.yaml`の`name`フィールドと一致させる。
...
**何？**：管理グループのメタデータを保存する場所

実は管理グループ自体は場所を持たないんだけど、ポリシー割り当てのデプロイメント用。
...
多くのポリシーが「Log Analyticsにログ送信」を要求するから、ワークスペースIDをここで一元管理。
...
## Part 4: モジュール呼び出し（main.management.tf）
...
Chapter 5で見た`locals.tf`で処理された設定が渡される。
...
- VMを作ったら自動でMonitoring Agent追加
- Diagnostic Settings自動設定
...
- アーキテクチャ定義（YAML）で階層構造を定義
- アーキタイプ（YAML）でポリシーセットを定義
- ポリシーは親から子に継承される
- `policy_assignments_to_remove`でデフォルトポリシーを無効化できる

次のChapterでは、管理リソース（Log Analytics、DCR、Managed Identity）の作成を見ていきます。
監視とログ収集の基盤です。
...
### 問題1
Azure Landing Zonesの管理グループ階層で、  
最上位の管理グループは何ですか？

### 問題2
`azurerm_management_group_policy_assignment`リソースで、  
ポリシーを割り当てる対象を指定するプロパティは何ですか？

### 問題3
次のうち、Azure Landing Zonesの標準管理グループはどれですか？
...
`root_id = "contoso"`で指定した名前がルート管理グループになります。

### 答え2
**`management_group_id`プロパティ**です。

```hcl
resource "azurerm_management_group_policy_assignment" "example" {
  name                 = "policy-assignment"
  policy_definition_id = "..."
  management_group_id  = azurerm_management_group.root.id  # ← これ
}
```

どの管理グループにポリシーを割り当てるかを指定します。

### 答え3
**A（platform）とC（connectivity）**が正解です。
...
B（production）とD（testing）は標準には含まれません。

---

**所要時間**: 50分  
**難易度**: ★★★★☆  
**前**: [07_リソースグループ.md](./07_リソースグループ.md)  
