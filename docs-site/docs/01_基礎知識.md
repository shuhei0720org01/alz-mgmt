...
変数を使えます。マップやリストを使うことで複雑なデプロイ処理を実現します。

変数は「variables.tf」というファイルにまとめることが多いです。
...
変数を渡すには、terraform.tfvarsまたはterraform.auto.tfvarsまたはterraform.tfvars.jsonというファイルに定義して渡します。
...
- terraform.tf：terraformの設定ファイル
- main.tf：リソース本体を定義
- variables.tf：変数を定義
- locals.tf：変数をいじって組み立て
- terraform.tfvars（terraform.auto.tfvars）：渡す変数を記載
- outputs.tf：出力を定義
