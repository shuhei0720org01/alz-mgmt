...
- テナントは、1社1テナントを強く推奨します。理由は後からテナントをまとめるのがすごく大変だからです。あと、entra ID P2のライセンスが二重でかかったり、DefenderやAzureポリシーのガバナンスが分かれちゃったり。
...
- 本番、開発などで管理グループを分けないのが推奨構成です。理由は管理グループの階層が深層化し管理が複雑になる上に、本番と開発でガバナンスを分ける効果が薄いため。
- CAFではサブスクリプションごとに環境を分けるのが推奨です。Microsoftの環境もこうなっているようです。

※サンドボックスは別管理グループに分けます。
...
- ERはリージョン冗長で配置
- HUBもリージョン冗長で配置。それに伴って、AzureFirewall、VPN Gatewayもリージョン冗長で配置することになる。
- アウトバウンドやER向けの通信はAzureFirewallを通す
- すべてのネットワークで、VNETフローログを有効化
- DNSはHUBに集約し、プライベートエンドポイントのレコード登録を自動化
- Azure Virtual Network Managerを使用して、NSGの禁止ルールを定義
- OnlineランディングゾーンのSpokeは会社ネットワークに接続できないよう制限し、利用者が自由にネットワークを設計できるようにする
- CorpランディングゾーンのSpokeは会社ネットワーク(HUB)に接続するため、ネットワーク設定を管理
...
- ログは単一のLog Analyticsワークスペースにすべて集約する。理由はSOCやインシデント対応の横断分析のため。
