...
- ✅ **1. Azure Billing and Active Directory Tenant**（課金とテナント - リージョン設定）
- ✅ **3. Resource Organization**（リソース整理 - リソースグループ構成）
- ✅ **4. Network Topology and Connectivity**（ネットワーク構成 - Hub-and-Spoke/Virtual WAN設定）
- ✅ **6. Management and Monitoring**（管理と監視 - Log Analytics設定）
...
次の Part 3 からは、IPアドレスやFirewallの設定が出てきます。

---

## Part 3: 設定値のアドレス空間

ここから先は、ネットワークの細かい設定が続く。

### 3-1. Firewallの設定

```hcl title="プライマリFirewallの設定"
...
**選択肢**
- `Basic`：安い（月¥30,000程度）、機能少ない
- `Standard`：標準（月¥170,000程度）、脅威インテリジェンスあり
- `Premium`：高機能（月¥100,000 + Premium課金）、TLS検査あり
...
```hcl title="Firewall SubnetのIP範囲"
primary_firewall_subnet_address_prefix = "10.0.128.0/26"
```

**これは何？**
Firewallが使うSubnetのIPアドレス範囲。
...
**zones** は **Availability Zones**（可用性ゾーン）のことです。

Azureでは、リソースを複数のゾーン（物理的に離れたデータセンター）に分散配置することで、  
...
**US East等のゾーン対応リージョンでは**:
```hcl title="ゾーン対応リージョンでの設定例"
primary_firewall_client_public_ip_zones = ["1", "2", "3"]  # ←3つのゾーンに分散
```
と設定すると、3つのゾーンに分散配置されて高可用性になります。
...
なんで2個？
VPN GatewayはActive-Active構成（2台同時稼働）にできるから。
- 1台目：`-001`
- 2台目：`-002`
...
```
10.0.0.0/16（Regional全体: 65,536個のIPアドレス）
...
| 用途 | CIDR | IP範囲 | 使える数 |
|------|------|--------|----------|
| Regional全体 | `10.0.0.0/16` | 10.0.0.0 - 10.0.255.255 | 65,536個 |
...
2. **management_ip_configuration**：管理用（Basic SKU
