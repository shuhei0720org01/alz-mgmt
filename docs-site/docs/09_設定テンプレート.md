...
![テンプレート変数の置換処理](./img/diagrams/template-processing.svg)


---

## 🏷️ はじめに：テンプレートエンジンの正体
...
- regionsモジュールでリージョン情報を取得
- client_configで現在のAzure認証情報を取得
- 10ステップの段階的な変数置換を実行
- 各ステップで前のステップの結果を活用
- JSON処理でブール値を保護
- 最終的に全設定ファイルの変数を置換
...
- 段階的処理で10ステップの変数置換
- ブール値はJSON処理で保護（true/falseが文字列にならない）
- coalesceで4段階のフォールバック（優先順位付き）
- for式で全設定に一括処理
- 階層的置換が可能（後の変数が前の変数を参照）
...
### ❓ 問題1
config-templatingモジュールの主な役割は何ですか？

### ❓ 問題2
以下の変数プレースホルダーは、どのように置換されますか？
...
### ❓ 問題3
`locals.config.tf`で行われる10ステップの処理のうち、最初の3ステップは何をしていますか？

### ❓ 問題4
なぜブール値（true/false）をJSON処理する必要があるのですか？
...
config-templatingモジュールの主な役割は、設定ファイル内の変数プレースホルダー（`$${変数名}`）を実際の値に置換することです。
...
これにより、同じテンプレートを使って異なる環境を簡単に構築できます。

### 💡 答え2
`$${starter_location_01}`は、`variables.tf`で定義された`starter_locations`配列の最初の要素に置換されます。
...
ブール値を通常の文字列置換で処理すると、`true`が`"true"`（文字列）になってしまうからです。
...
enabled = $${some_boolean}
↓ JSON処理で
enabled = true  # ✅ ブール値のまま
```

Terraformはブール値と文字列を厳密に区別するため、正しい型を保つ必要があります。

---

次のChapterでは、リソースグループの作成を見ていきます。実際のAzureリソースを作る最初のステップだよ。

---

**所要時間**: 60分  
**難易度**: ★★★★☆  
**前**: [07_ローカル変数.md](./07_ローカル変数.md)  
