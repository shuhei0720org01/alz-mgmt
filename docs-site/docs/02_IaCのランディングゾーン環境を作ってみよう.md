...
```powershell title="ALZ Moduleのインストール(バージョン固定するためにちょっと細工してます)"
# PowerShell 7.4+ で実行
Install-PSResource ALZ -Version 6.0.5 -Repository PSGallery -TrustRepository
$t="$HOME\Documents\PowerShell\Modules\ALZ\6.0.5\Private\Tools\Test-Tooling.ps1";Copy-Item $t "$t.bak" -Force
$l=Get-Content $t;$l[230]=$l[230]-replace '"Failure"','"Warning"';$l[232]='$hasFailure = $false'
Set-Content -Path $t -Value $l -Encoding utf8
Remove-Module ALZ -Force -ErrorAction SilentlyContinue;Import-Module ALZ -RequiredVersion 6.0.5 -Force
```
