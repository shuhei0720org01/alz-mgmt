...
## Part 1: 設定ファイル（platform-landing-zone.auto.tfvars）の実際

ここからも**実際のファイル**を見ていくよ。

### management_resource_settings

[platform-landing-zone.auto.tfvars](../platform-landing-zone.auto.tfvars#L185-L250)を開いてみよう。185行目から250行目あたりに管理リソースの設定が書いてある。
...
management_resource_settings = {
  ...
  # Names for the management resources (optional)
  ...
  # User Assigned Managed Identity for Azure Monitor Agent (optional)
  ...
  # Log Analytics Workspace settings (optional)
  ...
  # Sentinel (SIEM/SOAR) settings (optional)
...
見てわかるように、ほとんどの値が**`$${変数名}`**という形式になってる。これは**変数置換システム**だよ。
...
### 実際の変数定義（custom_replacements.names）

同じ[platform-landing-zone.auto.tfvars](../platform-landing-zone.auto.tfvars#L41-L80)ファイルの41行目から80行目あたりを見てみよう。

```hcl title="platform-landing-zone.auto.tfvars（custom_replacements.names）"
custom_replacements = {
  names = {
...
次は、実際のモジュール呼び出しを見ていこう。

### main.management.tfの実際

[main.management.tf](../main.management.tf#L1-L50)を開いてみよう。
...
[modules/management_resources/main.tf](../modules/management_resources/main.tf#L1-L30)を開いてみよう。
...
[modules/management_resources/variables.tf](../modules/management_resources/variables.tf#L1-L50)を開いてみよう。
...
[modules/management_resources/outputs.tf](../modules/management_resources/outputs.tf)
