# Chapter 09: 管理リソース（Log Analytics、DCR、Managed Identity）

!!! info "この章で学ぶこと"
    管理リソースの全体像を完全理解します：

    1. Log Analytics Workspace、DCR、Managed Identityの役割
    2. platform-landing-zone.auto.tfvarsの設定解説
    3. modules/management_resources/内部実装
    4. Azure/avm-ptn-alz-management公式モジュール完全解説
    5. Data Collection Rules（DCR）の3種類詳細
    6. 実践：カスタムDCRの追加方法

---

## Part 1: 設定ファイルの解説（platform-landing-zone.auto.tfvars）

`platform-landing-zone.auto.tfvars`の`management_resource_settings`ブロックを見ていこう。
...
管理リソース（Log Analytics、DCR、Managed Identity）の設定を1つのオブジェクトにまとめてる。

#### location

```hcl title="管理リソースのデプロイ先"
location = "japaneast"
```

**何してる？**

Log Analyticsワークスペースとその関連リソースを作る場所。

**重要**：

- 監視対象のリソースとは別のリージョンでもOK
- ただしデータ転送コストを考えると、監視対象と同じリージョンが一般的
...
Log Analyticsワークスペースの名前を指定してる。

**命名規則**：

- `law-`：Log Analytics Workspaceの略
- `management`：用途（管理リソース）
- `japaneast`：場所
...
管理リソース用のリソースグループ名を指定してる。

**このRGの中に入るもの**：

- Log Analytics Workspace
- Data Collection Rules（3つ）
...
Azure Monitor Agent（AMA）用のManaged Identityを定義してる。

**AMAって何？**

VMにインストールするエージェント。ログやメトリクスを収集してLog Analyticsに送る。
...
パスワード不要で認証できるのが便利。VMを作るたびにパスワード設定しなくていい。
...
3種類のData Collection Rule（DCR）を定義してる。
...
## Part 2: ローカルモジュール（modules/management_resources/）

`main.management.tf`から呼ばれるローカルモジュールを見ていこう。

### main.management.tf での呼び出し
...
- `count`で管理リソースの作成を制御
- `local.management_resource_settings`を渡す
- `azurerm.management`プロバイダを使ってManagementサブスクリプションに作成

### modules/management_resources/main.tf

```hcl title="公式モジュールの呼び出し"
module "management_resources" {
  ...
  automation_account_name = coalesce(
    var.management_resource_settings.automation_account_name,
    "aa-management-${var.management_resource_settings.location}"
  )
...
Azure公式の`avm-ptn-alz-management`モジュールを呼び出してる。約25個のパラメータを渡している。

**重要なパラメータ**：

- **location**：必須。管理リソースを作る場所
- **log_analytics_workspace_name**：`coalesce`でデフォルト値を設定
- **resource_group_name**：`coalesce`でデフォルト値を設定
- **automation_account_name**：`coalesce`でデフォルト値を設定
- **data_collection_rules**：3種類のDCRの設定
- **user_assigned_managed_identities**：Managed Identityの設定
- **linked_automation_account_creation_enabled**：`false`でAutomation Accountを作らない
- **resource_group_creation_enabled**：`true`でRGも自動作成
...
- 1つ目の値（ユーザー指定の名前）が`null`でなければそれを使う
- `null`だったら2つ目の値（デフォルト名）を使う

**例**：

```text title="coalesceの動作例"
# ユーザーが名前を指定した場合
log_analytics_workspace_name = "my-custom-law"
↓
結果：my-custom-law

# ユーザーが名前を指定しなかった場合（null）
log_analytics_workspace_name = null
↓
結果：law-management-japaneast
```

これで、ユーザーが名前を指定しなくても自動でいい感じの名前がつく。

### modules/management_resources/variables.tf

```hcl title="management_resource_settingsの型定義"
...
`management_resource_settings`の複雑なオブジェクト型を定義してる。約150行の定義。
...
1. **locationのみ必須**、他はoptional
2. **自動生成される名前**：未指定なら`coalesce`で自動生成
3. **デフォルト値**：`optional(bool, true)`の形式でデフォルト指定
4. **ネストしたオブジェクト**：DCRやSentinelは複雑な構造

### modules/management_resources/outputs.tf

```hcl title="モジュールのアウトプット"
...
<!-- TODO: スクリーンショット追加 - GitHubリポジトリとVS Codeを並べて表示 -->
...
management_resource_settings = {
  ...
  data_collection_rules = { change_tracking = { ... }, ... }
}
↓
...
↓
作成されるリソース：
  - rg-management（Resource Group）
  - law-management-japaneast（Log Analytics Workspace）
  - dcr-change-tracking（Change Tracking DCR）
  - dcr-vm-insights（VM Insights DCR）
  - dcr-defender-sql（Defender for SQL DCR）
  - id-ama-japaneast（User Assigned Managed Identity）
  - OMSGallery/ContainerInsights（Log Analytics Solution）
  - OMSGallery/VMInsights（Log Analytics Solution）
↓
outputs.tf（アウトプット）
...
1. **設定ファイル**：`platform-landing-zone.auto.tfvars`で設定
2. **ローカル変数**：`locals.tf`で展開
3. **モジュール呼び出し**：`main.management.tf`からローカルモジュール呼び出し
4. **ローカルモジュール**：`modules/management_resources/`から公式モジュール呼び出し
5. **公式モジュール**：`Azure/avm-ptn-alz-management`で実リソース作成
6. **アウトプット**：作成したリソースIDをアウトプット
7. **ポリシー連携**：ポリシーのデフォルト値に埋め込み
...
### パターン3: 複数のManaged Identity

```hcl title="複数のManaged Identityを作成"
management_resource_settings = {
  location = "japaneast"
  log_analytics_workspace_name = "law-management-japaneast"
  resource_group_name = "rg-management"
...
現在の`modules/management_resources/main.tf`は`user_assigned_managed_identities.ama`のみをサポートしてる。複数のManaged Identityを作成するには、公式モジュールのvariablesを調整する必要がある。

### パターン4: BYO Log Analytics Workspace

```hcl title="既存のLog Analyticsを使う"
# 既存のLog Analytics WorkspaceのIDを指定
module "management_resources" {
  source  = "Azure/avm-ptn-alz-management/azurerm"
  version = "0.9.0"
  
  location                                 = "japaneast"
  resource_group_name                      = "rg-existing-management"
  automation_account_name                  = "aa-management-japaneast"
  
  log_analytics_workspace_creation_enabled = false  # ←作成しない
  log_analytics_workspace_id               = "/subscriptions/xxx/resourceGroups/rg-existing/providers/Microsoft.OperationalInsights/workspaces/law-existing"
  resource_group_creation_enabled          = false  # ←既存RG使う
...
既存のLog Analyticsワークスペースを使う（BYO: Bring Your Own）。

**使い道**：

- 複数のLanding Zonesで1つのLog Analyticsを共有
- 既存の運用環境に統合
...
1. **Log Analytics Workspace**：全てのログを集約する中央集約ストア
2. **Data Collection Rules（DCR）**：どのログをどう収集するかのルール定義
3. **Managed Identity**：パスワード不要のセキュアな認証
...
### 問題1
Log Analytics Workspaceの主な役割は何ですか？
...
### 答え1
Log Analytics Workspaceの主な役割は、**すべてのログとメトリクスを集約する中央集約ストア**です。

具体的には：
- VM、コンテナ、Azureサービスからのログを収集
- 長期保存（デフォルト30日、最大730日）
- KQLクエリで分析・検索
- Azure MonitorやSentinelのデータソース
...
### 答え4
BYO（Bring Your Own）パターンは、**既存のリソースを再利用するパターン**です。

**例**：
```hcl
log_analytics_workspace = {
  enabled      = false  # 新規作成しない
  existing_resource_id = "/subscriptions/.../workspaces/law-existing-001"
}
```
