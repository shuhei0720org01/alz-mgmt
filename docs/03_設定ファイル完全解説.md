...
**Azureの推奨命名規則に従ってる**から、そのまま使えばOK。
...
これは**CIDR表記**（サイダー表記）と呼ばれるIPアドレスの範囲を表す記法です。
...
**数字が小さいほど広い範囲**です。詳しく理解する必要はありません。  
...
**これは何？**
個別のリソースの**フルパス**（Resource ID）を作ってる。
...
VPN Gatewayは**Active-Active**（2台構成）になってる。
ExpressRouteは1台だけ。
