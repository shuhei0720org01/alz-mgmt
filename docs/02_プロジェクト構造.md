...
**注意**: `.gitignore`に入ってる（Gitにコミットされない）
...
### 答え2
**variables.tf**は外部から渡される変数を定義します（入力パラメータ）。**locals.tf**は内部で計算される変数を定義します（計算結果）。
...
### 答え4
**count**は数値でリソースを作ります（`count = 3`なら3個）。インデックスは`count.index`で0,1,2...

**for_each**はマップやセットでリソースを作ります。各要素に名前があり、`each.key`や`each.value`でアクセスできます。
...
**for_each**のほうが、リソースの追加・削除時に他のリソースに影響しないため推奨されます。
