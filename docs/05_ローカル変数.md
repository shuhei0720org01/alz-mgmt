...
**例えるなら**：
- `variables.tf`：材料（卵、砂糖、小麦粉）
- `locals.tf`：下ごしらえ（卵を溶く、材料を混ぜる）
- `main.tf`：調理（オーブンで焼く）

変数をそのまま使うんじゃなくて、**計算・加工してから使う**。
...
でも、モジュール使ってる場合は**明示的に依存関係を書く**必要がある。
...
**これは何？**
管理グループにポリシーを割り当てる前に、**他のリソースを先に作る**ための仕組み。
...
### 答え3
**三項演算子**を使って、環境が本番（prod）かどうかを判定しています。
...
**次のChapter**で、`modules/config-templating/`を見ていきます。
